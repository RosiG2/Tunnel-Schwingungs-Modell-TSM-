#!/usr/bin/env python3
"""TSM compact operations runner v8.6.
...
def validate_operations(bundle: dict) -> tuple[dict, dict]:
    ...
    if policies.get("instruction_correspondence_audit_required_for_release") is not True:
        raise ValueError("v8.6 requires instruction-correspondence release auditing.")
    ...
    if grounding_policy.get("schema_id") != "tsm.module-grounding-policy.v1":
        raise ValueError("v8.6 preserves canonical module-grounding policy v1.")
    ...
    if broadening_policy.get("schema_id") != "tsm.retrieval-broadening-policy.v2":
        raise ValueError("v8.6 preserves retrieval broadening policy v2.")
    if broadening_policy.get("required_for_material_tsm_synthesis") is not False:
        raise ValueError("Open material TSM synthesis may not require a mandatory counterfind pass in v8.6.")
    ...
    if broadening_policy.get("required_for_hard_scientific_audit_or_high_stakes_claims") is not False:
        raise ValueError("Scientific validity countercheck must not be embedded as a pre-core retrieval gate in v8.6.")
    if broadening_policy.get("scientific_countercheck_is_part_of_korpus06_final_comment") is not True:
        raise ValueError("Scientific countercheck must belong to the final Korpus-06 comment in v8.6.")
    ...
    if synthesis_policy.get("schema_id") != "tsm.answer-synthesis-guidance-policy.v3":
        raise ValueError("v8.6 requires answer synthesis guidance v3.")
    ...
    if end_comment.get("required_for_all_substantive_tsm_answers") is not True or end_comment.get("position") != "last_visible_part_of_answer":
        raise ValueError("v8.6 requires the Korpus-06 scientific comment as the last visible answer phase.")
    ...
    if synthesis_policy.get("result_reverse_logic_available") is not True:
        raise ValueError("Result-based reverse logic must be available in v8.6.")
    ...
    if exploration.get("schema_id") != "tsm.exploratory-research-policy.v2":
        raise ValueError("v8.6 preserves exploratory research policy v2.")
    ...
    if set(comparison.get("epistemic_forms", {})) != required_epistemic_forms:
        raise ValueError("v8.6 preserves the five orthogonal epistemic forms.")
    ...
    if len(provenance.get("entries", [])) < 2:
        raise ValueError("v8.6 provenance registry must contain the current TSM-112 and TSM-111/112/113 curation anchors.")
...
def ai_execution_order_audit(root: Path, bundle: dict) -> dict:
    """Audit v8.6 grounding-first TSM synthesis with final non-rewriting scientific comment."""
    checks: dict[str, bool] = {}
    details: dict[str, object] = {}
    try:
        ...
        expected_workflow = [
            ...
            "identify_epistemic_goal_and_candidate_actual_modules",
        ...
        required_true = {
            ...
            "exploration_and_knowledge_opening_primacy_for_open_tsm_questions","grounded_estimation_is_legitimate_distinct_epistemic_stage",
        ...
        details.update({"workflow":system.get("workflow",[]),"materiality_horizon":system.get("materiality_horizon",[]),"note":"v8.6 preserves v8.5 grounding/retrieval and separates TSM core synthesis from the final non-rewriting scientific Korpus-06 comment; live model behavior still requires dialogue tests."})
    except Exception as exc:
        return {"status":"FAIL","checks":checks,"details":details,"error":str(exc)}
    status = "PASS" if checks and all(checks.values()) else "FAIL"
...
def module_grounding_audit(root: Path, bundle: dict) -> dict:
    """Audit preserved v8.5 canonical module-grounding and source-reset architecture in v8.6.
    ...
    try:
        ...
        companion_path = root / str(companion_cfg.get("expected_filename", ""))
        ...
        checks["companion_grounding_trigger"] = "müssen die einschlägigen tatsächlichen Modultexte des aktiven v8.6-Korpus vor der darauf gestützten Synthese herangezogen sein" in companion_text
...
def synthesis_guidance_audit(root: Path, bundle: dict) -> dict:
    ...
    try:
        ...
        checks["research_preference_matches_goal"]=connection.get("research_preference")=="match_next_step_to_epistemic_goal_open_exploration_or_hard_scientific_test"
...
def instruction_correspondence_audit(root: Path, bundle: dict) -> dict:
    """Static cross-file audit for v8.6 instruction correspondence; not a live LLM behavior proof."""
    checks: dict[str, bool] = {}
    details: dict[str, object] = {}
    try:
        ...
        companion=(root / str(cfg.get("expected_filename",""))).read_text(encoding="utf-8")
        system=bundle.get("ai_synthesis_system",{})
        ...
        checks["science_as_input_not_removed"]=(
            ...
            and "Fachwissen darf die Analyse und fundierte Schätzung informieren" in companion
        )
        ...
        checks["korpus06_final_comment_consistent"]=(
            ...
            and "**Ausführungsrolle v8.6:**" in k06
            and "wissenschaftliche Prüfung nach Korpusdatei 06 wird erst auf die fertige TSM-Kernantwort angewandt" in companion
        )
        checks["no_retroactive_core_rewrite"]=(
            ...
            and "nicht rückwirkend kürzen, abschwächen oder umformulieren" in companion
        )
        ...
        checks["operative_method_use_distinct_from_scientific_validity_check"]=(
            "Operative Rechen-, Proxy-, 136D- und Evidenzregeln aus Korpusdatei 06" in companion
            and "die wissenschaftliche Geltungsprüfung bleibt Schlusskommentar" in companion
        )
        checks["theoretical_commentary_distinct_from_scientific_validation"]=(
            ...
            and "nicht mit der wissenschaftlichen Korpus-06-Schlusseinordnung zu verwechseln" in companion
        )
        ...
        checks["no_old_companion_science_auto_hardening_trigger"]=(
            "Bei harten wissenschaftlichen Arbeiten richtet sich die Bearbeitung dadurch automatisch stärker an Fachlogik" not in companion
            and "Externe Quellen können Außenfragen beantworten und TSM-Deutungen begrenzen." not in companion
        )
        ...
        details.update({
            ...
            "note":"Static cross-file consistency only; live retrieval and model weighting require deployment regression tests."
        })
    except Exception as exc:
        return {"status":"FAIL","checks":checks,"details":details,"error":str(exc)}
    status="PASS" if checks and all(checks.values()) else "FAIL"
    return {"status":status,"checks":checks,"details":details,"claim_scope":"static_instruction_correspondence_not_live_llm_behavior_proof"}
