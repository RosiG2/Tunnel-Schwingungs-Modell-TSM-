#!/usr/bin/env python3
"""TSM compact operations runner v8.4.
...
def validate_operations(bundle: dict) -> tuple[dict, dict]:
    ...
    if broadening_policy.get("schema_id") != "tsm.retrieval-broadening-policy.v2":
        raise ValueError("v8.4 requires retrieval broadening policy v2.")
    if broadening_policy.get("required_for_material_tsm_synthesis") is not False:
        raise ValueError("Open material TSM synthesis may not require a mandatory counterfind pass in v8.4.")
    ...
    if broadening_policy.get("required_for_hard_scientific_audit_or_high_stakes_claims") is not True:
        raise ValueError("Hard scientific or high-stakes claims require relevant countercheck/hardening.")
    ...
    if synthesis_policy.get("schema_id") != "tsm.answer-synthesis-guidance-policy.v2":
        raise ValueError("v8.4 requires answer synthesis guidance v2.")
    if synthesis_policy.get("synthesis_primacy") is not True or synthesis_policy.get("exploration_primacy") is not True:
        raise ValueError("Open TSM questions require exploration and synthesis primacy.")
    if synthesis_policy.get("relational_possibility_space_before_hardening") is not True:
        raise ValueError("The relational possibility space must be opened before default hardening.")
    ...
    if synthesis_policy.get("result_reverse_logic_available") is not True:
        raise ValueError("Result-based reverse logic must be available in v8.4.")
    ...
    if exploration.get("schema_id") != "tsm.exploratory-research-policy.v2":
        raise ValueError("v8.4 requires exploratory research policy v2.")
    ...
    if set(comparison.get("epistemic_forms", {})) != required_epistemic_forms:
        raise ValueError("v8.4 requires the five orthogonal epistemic forms.")
    ...
    if meta04_scope.get("not_default_exploration_gate") is not True or meta04_scope.get("hardening_limits_claim_not_possibility_space") is not True:
        raise ValueError("META-04 must remain a downstream hardening/audit layer, not an exploration gate.")
    ...
    if len(provenance.get("entries", [])) < 2:
        raise ValueError("v8.4 provenance registry must contain the current TSM-112 and TSM-111/112/113 curation anchors.")
...
def urf_rows(rows: list[dict]) -> list[dict]:
    """Validate either classic reverse-research reconstruction or v8.4 result-based reverse logic."""
    output = []
    for source in rows:
        row = dict(source)
...
def comparison_framework_audit(root: Path, bundle: dict) -> dict:
    ...
    try:
        ...
        checks["meta04_is_downstream_hardening_layer"] = (
            ...
            and meta04_scope.get("hardening_limits_claim_not_possibility_space") is True
            and "**Geltungshinweis v8.4 -- Härtung folgt Exploration**" in meta04_text
...
def ai_execution_order_audit(root: Path, bundle: dict) -> dict:
    """Audit v8.4 exploration-first execution architecture and reciprocal synthesis binding."""
    checks: dict[str, bool] = {}
    details: dict[str, object] = {}
    try:
        ...
        key_markers = [
            ...
            "Wissenschaftliche Härtung",
            "Mindesttiefe und knappe Integritätsprüfung",
        ]
        checks["execution_key_v84_markers_present"] = all(marker in key_text for marker in key_markers)
        ...
        expected_workflow = [
            ...
            "identify_epistemic_goal_retrieve_actual_modules_and_open_relational_possibility_space",
            ...
            "broaden_independently_when_material_gain_hardening_goal_or_far_reaching_new_synthesis_requires",
            "apply_scientific_evidence_and_protection_hardening_according_to_claim_goal",
            "check_subject_focus_relational_process_value_and_internal_claim_status",
            "condense_synthesis_and_offer_material_connection_preview_when_substantive",
        ]
        ...
        required_true = {
            ...
            "audit_does_not_replace_responsibility","audience_adaptation_may_not_change_claim_status","file06_governs_external_scientific_medical_technical_claims",
            ...
            "scientific_hardening_follows_claim_goal_not_default_idea_gate","independent_broadening_after_exploration_when_material_gain_expected",
            "broadening_searches_for_unseen_not_only_counterevidence","hard_scientific_or_high_stakes_claims_require_relevant_hardening",
        ...
        details.update({"workflow":system.get("workflow",[]),"materiality_horizon":system.get("materiality_horizon",[]),"note":"v8.4 exploration-first execution audit; actual model behavior still requires dialogue tests."})
    except Exception as exc:
        return {"status":"FAIL","checks":checks,"details":details,"error":str(exc)}
    status = "PASS" if checks and all(checks.values()) else "FAIL"
    return {"status":status,"checks":checks,"details":details,"claim_scope":"joint_ai_synthesis_exploration_primacy_grounded_estimation_reverse_logic_theoretical_commentary_and_conditional_hardening"}

def retrieval_counterfind_audit(root: Path, bundle: dict) -> dict:
    """Audit v8.4 independent broadening policy; command name retained for CLI compatibility."""
    checks: dict[str, bool] = {}
    details: dict[str, object] = {}
    try:
        system = bundle.get("ai_synthesis_system", {})
        ...
        checks["hard_claim_hardening_required"] = policy.get("required_for_hard_scientific_audit_or_high_stakes_claims") is True
        ...
        checks["reading_key_no_mandatory_counterfind"] = "kein Pflichtfilter jeder Idee" in key_text
...
def synthesis_guidance_audit(root: Path, bundle: dict) -> dict:
    """Audit v8.4 exploration primacy, grounded estimation, reverse logic and claim-goal-specific hardening."""
    checks: dict[str, bool] = {}
    details: dict[str, object] = {}
    try:
        system=bundle.get("ai_synthesis_system",{})
        ...
        key_text=key_path.read_text(encoding="utf-8") if key_path.is_file() else ""
        checks["policy_registry_present"]=bool(policy)
        checks["policy_schema_v2"]=policy.get("schema_id")=="tsm.answer-synthesis-guidance-policy.v2"
        ...
        checks["possibility_space_before_hardening"]=policy.get("relational_possibility_space_before_hardening") is True
        ...
        checks["reading_key_hardening_claim_marker"]="wie stark etwas behauptet werden darf" in key_text
        ...
        checks["workflow_order_exploration_before_hardening"]=checks["workflow_steps_present"] and workflow.index(required[0]) < workflow.index(required[1]) < workflow.index(required[2]) < workflow.index(required[3])
        details.update({"workflow":workflow,"principle":policy.get("principle")})
    except Exception as exc:
        return {"status":"FAIL","checks":checks,"details":details,"error":str(exc)}
    status="PASS" if checks and all(checks.values()) else "FAIL"
    return {"status":status,"checks":checks,"details":details,"claim_scope":"answer_synthesis_exploration_primacy_grounded_estimation_reverse_logic_theoretical_commentary_and_conditional_hardening"}
...
def dialog_routing_audit(root: Path, bundle: dict) -> dict:
    ...
    try:
        ...
        checks["workflow_ends_with_compact_integrity_and_condensation"] = workflow[-1:] == ["condense_synthesis_and_offer_material_connection_preview_when_substantive"]
