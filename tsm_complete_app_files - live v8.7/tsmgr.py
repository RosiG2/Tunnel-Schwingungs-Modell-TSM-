#!/usr/bin/env python3
"""TSM compact operations runner v8.7.
...
def validate_operations(bundle: dict) -> tuple[dict, dict]:
    ...
    if companion.get("external_to_active_file_budget") is not True:
        raise ValueError("The application-hint companion must remain external to the active file budget.")
    if companion.get("deployment_location") != "customgpt_instruction_field":
        raise ValueError("The application-hint companion must be deployed in the CustomGPT instruction field.")
    if companion.get("is_knowledge_or_corpus_file") is not False or companion.get("counts_against_active_file_budget") is not False:
        raise ValueError("The application-hint companion must not be treated as a Knowledge/corpus file.")
    if companion.get("must_not_be_uploaded_as_active_knowledge_file") is not True:
        raise ValueError("Duplicate companion upload as an active Knowledge file must be forbidden.")
    if companion.get("runtime_knowledge_presence_expected") is not False:
        raise ValueError("The external application hint must not be expected inside runtime Knowledge.")
    if companion.get("absence_from_knowledge_is_expected_not_error") is not True:
        raise ValueError("Absence of the external application hint from Knowledge must be an expected non-error state.")
    if companion.get("runner_can_audit_live_instruction_field_presence") is not False:
        raise ValueError("The runner must not claim it can audit live CustomGPT instruction-field presence.")
    if companion.get("corpus_audits_must_not_require_companion_file") is not True:
        raise ValueError("Corpus-only audits must not require the external application-hint file.")
    if companion.get("runtime_missing_companion_file_must_not_create_corpus_status_error") is not True:
        raise ValueError("Missing external application-hint file in corpus root must not create a corpus-status error.")
    active_paths = {str(entry.get("path")) for entry in bundle.get("manifest", {}).get("active_files", [])}
    if str(companion.get("expected_filename")) in active_paths:
        raise ValueError("The application-hint companion may not appear in manifest.active_files.")
    ...
    if int(release.get("active_file_count", -1)) != int(budget.get("active_files", -2)):
        raise ValueError("Release metadata active_file_count is inconsistent with file budget.")

    book_reference = require(bundle, "public_book_reference")
    if book_reference.get("current_corpus_version") != release_version:
        raise ValueError("Public book reference current_corpus_version is inconsistent.")
    if book_reference.get("semantic_authority") is not False:
        raise ValueError("The public book reference must not be declared as module semantic authority.")
    if book_reference.get("explicit_book_question_policy") != "book_is_primary_for_wording_structure_chapters_and_pages":
        raise ValueError("Public book reference explicit-question policy is incomplete.")
    if book_reference.get("general_tsm_question_policy") != "current_module_texts_remain_primary":
        raise ValueError("Public book reference general-question policy is incomplete.")
    if book_reference.get("conflict_policy") != "disclose_material_conflict_current_corpus_prevails_for_current_basis":
        raise ValueError("Public book reference conflict policy is incomplete.")
    if book_reference.get("perspective_variation_policy") != "narrower_broader_or_narrative_book_framing_need_not_be_announced_unless_material":
        raise ValueError("Public book reference perspective-variation policy is incomplete.")

    policies = require(bundle, "policies")
    if policies.get("claim_status_internal_by_default") is not True:
        raise ValueError("Claim/source status must remain internally determined by default.")
    if policies.get("normal_tsm_answer_is_coherent_unit") is not True:
        raise ValueError("Normal TSM answers must be allowed to remain coherent units.")
    if policies.get("exploratory_e1_e2_modeling_with_model_set_proxies_allowed") is not True:
        raise ValueError("Exploratory E1/E2 modeling with transparent model-set proxies must be allowed.")
    if policies.get("fail_closed_limits_claim_not_exploration") is not True:
        raise ValueError("Fail-closed must limit claim strength rather than exploratory modeling.")
    if policies.get("sensitive_domain_boundary_visible_once_per_material_block") is not True:
        raise ValueError("Sensitive-domain external-fact/TSM boundary visibility floor is required.")
    if policies.get("current_method_layer_overrides_older_book_methodology") is not True:
        raise ValueError("Current method and operations layer must override older book methodology for live research rules.")
    if policies.get("korpus06_scientific_evaluation_is_final_comment_for_all_substantive_tsm_answers") is not True:
        raise ValueError("Korpus 06 scientific evaluation must be a final comment for substantive TSM answers.")
    if policies.get("korpus06_final_comment_must_not_rewrite_tsm_core") is not True:
        raise ValueError("Korpus 06 final comment must not rewrite the TSM core.")
    if policies.get("factual_error_correction_is_not_korpus06_rewrite_permission") is not True:
        raise ValueError("Factual correction must not become Korpus-06 rewrite permission.")
    if policies.get("scientific_information_may_inform_exploration_and_grounded_estimation") is not True:
        raise ValueError("Science must remain usable as input to exploration and grounded estimation.")
    if policies.get("instruction_correspondence_audit_required_for_release") is not True:
        raise ValueError("v8.7 requires instruction-correspondence release auditing.")
    artifact_registry = require(bundle, "artifact_reference_registry")
    if artifact_registry.get("fail_on_unclassified_reference") is not True:
        raise ValueError("Artifact-reference audit must fail on unclassified file references.")
    research_spec = require(bundle, "operation_specs.research_scenario_evaluate")
    if research_spec.get("classification_allowed") is not False or research_spec.get("calculation_status") != "DEMO_ONLY":
        raise ValueError("Research-scenario evaluator must be DEMO_ONLY and classification-blocked.")

    ai_system = require(bundle, "ai_synthesis_system")
    ai_policies = ai_system.get("policies", {})
    if ai_policies.get("default_free_exploratory_mode_for_substantive_tsm_questions") is not True:
        raise ValueError("Substantive TSM questions must default to free module-grounded exploration.")
    if ai_policies.get("explicit_user_trigger_for_exploration_required") is not False:
        raise ValueError("Default TSM exploration must not require an explicit user trigger.")
    if ai_policies.get("epistemic_goal_modulates_depth_not_exploration_activation") is not True:
        raise ValueError("Epistemic goal may modulate depth but must not switch default exploration on or off.")
    if ai_policies.get("scientific_technical_or_data_topic_does_not_disable_default_exploration") is not True:
        raise ValueError("Scientific, technical or data topics must not disable default TSM exploration.")
    ...
    grounding_policy = require(ai_system, "module_grounding_policy")
    if grounding_policy.get("schema_id") != "tsm.module-grounding-policy.v1":
        raise ValueError("v8.7 preserves canonical module-grounding policy v1.")
    if grounding_policy.get("required_before_material_synthesis") is not True:
        raise ValueError("Material current-BASIS synthesis requires canonical module grounding before synthesis.")
    if grounding_policy.get("status_values") != ["SATISFIED", "NOT_FOUND", "VERSION_CONFLICT", "SOURCE_UNAVAILABLE", "NOT_REQUIRED"]:
        raise ValueError("Module-grounding status values are incomplete or reordered.")
    reset = grounding_policy.get("source_reset", {})
    if reset.get("affected_ungrounded_synthesis_ceases_to_be_semantic_starting_point") is not True or reset.get("resynthesize_affected_branch_from_grounded_module_text") is not True:
        raise ValueError("Grounding errors must trigger source-reset branch resynthesis.")
    live = grounding_policy.get("live_retrieval_proof", {})
    if live.get("runner_can_prove_live_platform_chunk_retrieval") is not False or live.get("false_live_retrieval_guarantee_claim_forbidden") is not True:
        raise ValueError("Runner must not claim proof of live platform retrieval.")
    broadening_policy = require(ai_system, "retrieval_counterfind_policy")
    if broadening_policy.get("schema_id") != "tsm.retrieval-broadening-policy.v2":
        raise ValueError("v8.7 preserves retrieval broadening policy v2.")
    if broadening_policy.get("required_for_material_tsm_synthesis") is not False:
        raise ValueError("Open material TSM synthesis may not require a mandatory counterfind pass in v8.7.")
    if broadening_policy.get("independent_broadening_after_exploration_when_material_gain_expected") is not True:
        raise ValueError("Independent broadening after exploration must remain available when materially useful.")
    if broadening_policy.get("required_for_hard_scientific_audit_or_high_stakes_claims") is not False:
        raise ValueError("Scientific validity countercheck must not be embedded as a pre-core retrieval gate in v8.7.")
    if broadening_policy.get("scientific_countercheck_is_part_of_korpus06_final_comment") is not True:
        raise ValueError("Scientific countercheck must belong to the final Korpus-06 comment in v8.7.")
    if broadening_policy.get("independent_broadening_is_epistemic_not_scientific_validity_gate") is not True:
        raise ValueError("Independent broadening must remain epistemic rather than a scientific validity gate.")
    if broadening_policy.get("alternative_semantic_axis_required_when_broadening") is not True:
        raise ValueError("When broadening is used, an alternative semantic axis is required.")
    if broadening_policy.get("far_reaching_new_synthesis_requires_theoretical_commentary") is not True:
        raise ValueError("Far-reaching new TSM syntheses require independent theoretical commentary.")
    if int(broadening_policy.get("theoretical_commentary_minimum_independent_perspectives", 0)) < 1:
        raise ValueError("Theoretical commentary requires at least one materially independent perspective.")
    if broadening_policy.get("theoretical_commentary_result_open") is not True or broadening_policy.get("theoretical_commentary_is_not_mandatory_counterevidence_search") is not True:
        raise ValueError("Theoretical commentary must remain result-open and must not become a counterevidence mandate.")
    if broadening_policy.get("exhaustive_chunk_retrieval_guarantee_claim_forbidden") is not True:
        raise ValueError("The corpus may not claim a technical exhaustive-chunk retrieval guarantee.")

    synthesis_policy = require(ai_system, "answer_synthesis_policy")
    if synthesis_policy.get("schema_id") != "tsm.answer-synthesis-guidance-policy.v3":
        raise ValueError("v8.7 requires answer synthesis guidance v3.")
    if synthesis_policy.get("synthesis_primacy") is not True or synthesis_policy.get("exploration_primacy") is not True:
        raise ValueError("TSM questions require exploration and synthesis primacy.")
    if synthesis_policy.get("relational_possibility_space_before_scientific_end_comment") is not True:
        raise ValueError("The relational possibility space and TSM core must precede scientific end-comment evaluation.")
    if synthesis_policy.get("scientific_information_may_inform_reasoning") is not True:
        raise ValueError("Scientific information must remain available as input to TSM reasoning and grounded estimation.")
    end_comment=synthesis_policy.get("scientific_end_comment",{})
    if end_comment.get("required_for_all_substantive_tsm_answers") is not True or end_comment.get("position") != "last_visible_part_of_answer":
        raise ValueError("v8.7 requires the Korpus-06 scientific comment as the last visible answer phase.")
    if end_comment.get("must_not_rewrite_shorten_weaken_or_metaphorize_tsm_core") is not True:
        raise ValueError("The scientific end comment may not rewrite or weaken the TSM core.")
    if end_comment.get("factual_error_correction_remains_allowed_and_required") is not True:
        raise ValueError("Truthful factual correction must remain active despite phase separation.")
    if end_comment.get("factual_error_correction_is_not_korpus06_rewrite_permission") is not True:
        raise ValueError("Factual correction may not be used as a Korpus-06 backchannel into the TSM core.")
    if synthesis_policy.get("grounded_estimation_is_legitimate") is not True:
        raise ValueError("Grounded estimation must be recognized as a legitimate epistemic stage.")
    if synthesis_policy.get("result_reverse_logic_available") is not True:
        raise ValueError("Result-based reverse logic must be available in v8.7.")
    commentary = synthesis_policy.get("far_reaching_synthesis_theoretical_commentary", {})
    if commentary.get("required") is not True or int(commentary.get("minimum_materially_independent_perspectives", 0)) < 1:
        raise ValueError("Far-reaching synthesis theoretical commentary is missing or underspecified.")
    if commentary.get("result_open") is not True or commentary.get("counterevidence_not_required") is not True:
        raise ValueError("Theoretical commentary must be result-open rather than counterevidence-biased.")
    if synthesis_policy.get("subject_primacy") is not True:
        raise ValueError("The visible synthesis must preserve subject primacy.")
    if synthesis_policy.get("connection_preview", {}).get("mandatory") is not False:
        raise ValueError("Connection preview must remain optional, not a mandatory closing.")
    if synthesis_policy.get("connection_preview", {}).get("generic_more_research_needed_closing_forbidden") is not True:
        raise ValueError("Generic routine connection-preview closings must be forbidden.")

    exploration = require(ai_system, "exploratory_research_policy")
    if exploration.get("schema_id") != "tsm.exploratory-research-policy.v2":
        raise ValueError("v8.7 preserves exploratory research policy v2.")
    if exploration.get("grounded_estimation_allowed") is not True or exploration.get("grounded_estimation_assumptions_and_uncertainty_required") is not True:
        raise ValueError("Grounded estimation must be allowed with assumptions and uncertainty disclosed.")
    if exploration.get("grounded_estimation_need_not_be_collapsed_into_testable_hypothesis") is not True:
        raise ValueError("Grounded estimation must remain distinct from a testable hypothesis.")

    comparison = require(bundle, "comparison_calibration_framework")
    required_epistemic_forms = {"WIEDERGABE", "DEUTUNG", "HYPOTHESE", "FUNDIERTE_SCHAETZUNG", "DATENBEFUND"}
    if set(comparison.get("epistemic_forms", {})) != required_epistemic_forms:
        raise ValueError("v8.7 preserves the five orthogonal epistemic forms.")
    grounded_policy = require(comparison, "grounded_estimation_policy")
    if grounded_policy.get("epistemic_form") != "FUNDIERTE_SCHAETZUNG" or grounded_policy.get("claim_level_H_does_not_reclassify_as_hypothesis") is not True:
        raise ValueError("Grounded estimation must remain its own epistemic form even when claim_level is H.")
    meta04_scope = require(comparison, "meta04_scope_policy")
    if meta04_scope.get("not_default_exploration_gate") is not True or meta04_scope.get("scientific_audit_classifies_claim_not_possibility_space") is not True:
        raise ValueError("META-04 must remain a scientific classification/audit layer, not a TSM-synthesis gate.")

    reverse_logic = require(ai_system, "result_reverse_logic_policy")
    if reverse_logic.get("distinct_from_reverse_research_reconstruction") is not True:
        raise ValueError("Result-based reverse logic must remain distinct from reverse-research reconstruction.")

    provenance = require(bundle, "semantic_authority.module_claim_provenance_registry")
    if provenance.get("exhaustive_historical_reconstruction_required") is not False:
        raise ValueError("Slim provenance may not require exhaustive historical reconstruction.")
    if provenance.get("systematic_legacy_countersearch_required") is not False:
        raise ValueError("Slim provenance may not require systematic legacy countersearch.")
    if len(provenance.get("entries", [])) < 2:
        raise ValueError("v8.7 provenance registry must contain the current TSM-112 and TSM-111/112/113 curation anchors.")
    if len(provenance.get("entries", [])) < 3:
        raise ValueError("v8.7 provenance registry must contain the TSM-112 314 working-function curation anchor.")

    policies = require(bundle, "policies")
    if policies.get("TSM112_314_is_grounded_working_function_not_universal_cycle") is not True:
        raise ValueError("v8.7 must keep 314 as a grounded TSM working function, not a universal cycle.")
    if policies.get("TSM112_title_and_69_function_preserved_in_v8_7") is not True:
        raise ValueError("v8.7 must preserve the TSM-112 title and the 69-year function.")
    if policies.get("TSM112_pi100_is_symbolic_hint_not_mechanism") is not True:
        raise ValueError("v8.7 must keep π×100 as a symbolic hint rather than a mechanism claim.")
    if policies.get("TSM112_v8_7_does_not_close_historical_69_reconstruction") is not True:
        raise ValueError("v8.7 may not close the historical reconstruction of the 69-year reading.")
...
def validate_package(root: Path, bundle: dict) -> dict:
    ...
    grounding_framework = module_grounding_audit(root, bundle)
    if grounding_framework["status"] != "PASS":
        raise ValueError(f"Module grounding audit failed: {grounding_framework}")
    counterfind_framework = retrieval_counterfind_audit(root, bundle)
    if counterfind_framework["status"] != "PASS":
        raise ValueError(f"Retrieval counterfind audit failed: {counterfind_framework}")
    synthesis_framework = synthesis_guidance_audit(root, bundle)
    if synthesis_framework["status"] != "PASS":
        raise ValueError(f"Synthesis guidance audit failed: {synthesis_framework}")
    instruction_correspondence = instruction_correspondence_audit(root, bundle)
    if instruction_correspondence["status"] != "PASS":
        raise ValueError(f"Instruction correspondence audit failed: {instruction_correspondence}")
    ...
    book_reference = public_book_reference_audit(root, bundle)
    if book_reference["status"] != "PASS":
        raise ValueError(f"Public book reference audit failed: {book_reference}")
    artifact_references = artifact_reference_audit(root, bundle)
    if artifact_references["status"] != "PASS":
        raise ValueError(f"Artifact reference audit failed: {artifact_references}")
    research_scenarios = research_scenario_contract_audit(root, bundle)
    if research_scenarios["status"] != "PASS":
        raise ValueError(f"Research scenario contract audit failed: {research_scenarios}")
    ...
    return {
        ...
        "public_book_reference": book_reference,
        "artifact_reference_audit": artifact_references,
        "module_grounding_audit": grounding_framework,
        "retrieval_counterfind_audit": counterfind_framework,
        "synthesis_guidance_audit": synthesis_framework,
        "instruction_correspondence_audit": instruction_correspondence,
        "research_scenario_enforcement": research_scenarios,
    }
...
def _percentile_rank_against_sorted(values: list[float], x: float) -> float:
    """Average-rank percentile against a fixed sorted reference distribution."""
    if not values:
        raise ValueError("Reference distribution is empty.")
    lo = bisect.bisect_left(values, x)
    hi = bisect.bisect_right(values, x)
    if lo == hi:
        # Position a new value between adjacent empirical ranks.
        return clamp((lo + 0.5) / len(values), 0.0, 1.0)
    return clamp(((lo + 1 + hi) / 2.0) / len(values), 0.0, 1.0)


def research_scenario_rows(rows: list[dict], profile: dict, core_rows: list[dict]) -> list[dict]:
    """Evaluate explicitly exploratory E1/E2 proxy scenarios.

    This path is intentionally incapable of emitting a real classification.  It
    uses the canonical synthetic 136D core only as a reference distribution for
    hypothetical scenario ranks and zones.
    """
    allowed_origins = {"measured", "source_derived", "rule_derived", "normalized", "model_set"}
    eps = float(profile["numeric"]["epsilon_rad"])
    B_hi = float(profile["numeric"]["B_hi"])
    B_lo = float(profile["numeric"]["B_lo"])
    S_hi = float(profile["numeric"]["S_hi"])
    S_lo = float(profile["numeric"]["S_lo"])
    cap_q = float(profile["numeric"]["cap_quantile"])

    core_f = [as_float(r, "recommended_F_res") for r in core_rows]
    core_f = [x for x in core_f if x is not None]
    if not core_f:
        raise ValueError("Canonical core contains no recommended_F_res reference values.")
    core_cap = quantile(core_f, cap_q)
    core_fcap_sorted = sorted(min(x, core_cap) for x in core_f)
    core_s_sorted = []
    for r in core_rows:
        C = as_float(r, "recommended_C")
        dphi = as_float(r, "recommended_dphi_rad")
        tau = as_float(r, "recommended_tau")
        if None not in (C, dphi, tau) and C > 0 and tau > 0:
            core_s_sorted.append(max(abs(dphi), eps) / (C * tau))
    core_s_sorted.sort()
    if not core_s_sorted:
        raise ValueError("Canonical core contains no usable S reference values.")

    prepared: list[dict] = []
    family_variants: dict[str, set[str]] = {}
    for index, source in enumerate(rows, 1):
        row = dict(source)
        family = as_text(row, "scenario_family") or as_text(row, "scenario_id")
        variant = as_text(row, "variant_id")
        row["calculation_status"] = "DEMO_ONLY"
        row["classification_allowed"] = False
        row["real_classification_status"] = "forbidden_exploratory_scenario"
        row["evidence_maturity"] = "E2"
        row["claim_level_max"] = "H"
        row["reference_distribution"] = "TSM_136D_Core.csv:recommended"
        row["scenario_rank_scope"] = "synthetic_reference_distribution_not_empirical_calibration"
        row["scenario_family"] = family or ""
        row["variant_id"] = variant or ""

        errors = []
        if not family:
            errors.append("missing_scenario_family")
        if not variant:
            errors.append("missing_variant_id")
        if family and variant:
            family_variants.setdefault(family, set()).add(variant)

        vals = {name: as_float(row, name) for name in ("C", "dphi_rad", "tau")}
        if vals["C"] is None or not (0 < vals["C"] <= 1):
            errors.append("invalid_C")
        if vals["dphi_rad"] is None:
            errors.append("invalid_dphi_rad")
        if vals["tau"] is None or vals["tau"] <= 0:
            errors.append("invalid_tau")

        origins = {}
        for name in ("C", "dphi", "tau"):
            key = f"{name}_origin"
            value = (as_text(row, key) or "").lower()
            origins[name] = value
            if value not in allowed_origins:
                errors.append(f"invalid_{key}")
            basis_key = f"{name}_basis"
            if not as_text(row, basis_key):
                errors.append(f"missing_{basis_key}")

        model_set_fields = [name for name, origin in origins.items() if origin in {"normalized", "model_set"}]
        row["model_set_or_normalized_fields"] = ",".join(model_set_fields)
        row["proxy_provenance_complete"] = not any(e.startswith("invalid_") or e.startswith("missing_") for e in errors)

        if errors:
            row["research_scenario_status"] = "BLOCKED_INPUT_CONTRACT"
            row["research_scenario_errors"] = ";".join(errors)
            row["F_res_scenario"] = ""
            row["B_scenario_rank"] = ""
            row["S_scenario_rank"] = ""
            row["hypothetical_scenario_zone"] = ""
        else:
            C = vals["C"]
            dphi = vals["dphi_rad"]
            tau = vals["tau"]
            denom = max(abs(dphi), eps)
            fres = C * tau / denom
            fcap = min(fres, core_cap)
            sraw = denom / (C * tau)
            B = _percentile_rank_against_sorted(core_fcap_sorted, fcap)
            S = _percentile_rank_against_sorted(core_s_sorted, sraw)
            if B >= B_hi and S <= S_lo:
                zone = "K"
            elif B <= B_lo and S >= S_hi:
                zone = "F"
            else:
                zone = "R"
            row["F_res_scenario"] = fres
            row["F_cap_scenario"] = fcap
            row["B_scenario_rank"] = B
            row["S_scenario_rank"] = S
            row["hypothetical_scenario_zone"] = zone
            row["research_scenario_status"] = "EVALUATED_DEMO_ONLY_PENDING_VARIANT_CHECK"
            row["research_scenario_errors"] = ""
        prepared.append(row)

    # Variants are a hard part of the E2 contract.  A family with fewer than
    # two variants may show its arithmetic inputs but is not released as an
    # evaluated scenario family.
    ...
    for family, group in by_family.items():
        n_variants = len(family_variants.get(family, set()))
        zones = [str(r.get("hypothetical_scenario_zone", "")) for r in group if r.get("hypothetical_scenario_zone")]
        fres_values = [as_float(r, "F_res_scenario") for r in group]
        fres_values = [x for x in fres_values if x is not None]
        B_values = [as_float(r, "B_scenario_rank") for r in group]
        B_values = [x for x in B_values if x is not None]
        S_values = [as_float(r, "S_scenario_rank") for r in group]
        S_values = [x for x in S_values if x is not None]
        zone_set = sorted(set(zones))
        for row in group:
            row["scenario_family_variant_count"] = n_variants
            row["sensitivity_checked"] = n_variants >= 2
            row["scenario_family_zone_set"] = ",".join(zone_set)
            row["scenario_family_proxy_sensitive"] = len(zone_set) > 1
            row["scenario_family_F_res_min"] = min(fres_values) if fres_values else ""
            row["scenario_family_F_res_max"] = max(fres_values) if fres_values else ""
            row["scenario_family_B_min"] = min(B_values) if B_values else ""
            row["scenario_family_B_max"] = max(B_values) if B_values else ""
            row["scenario_family_S_min"] = min(S_values) if S_values else ""
            row["scenario_family_S_max"] = max(S_values) if S_values else ""
            if n_variants < 2 and row.get("research_scenario_status", "").startswith("EVALUATED_"):
                row["research_scenario_status"] = "BLOCKED_VARIANTS_REQUIRED"
                row["hypothetical_scenario_zone"] = ""
            elif row.get("research_scenario_status") == "EVALUATED_DEMO_ONLY_PENDING_VARIANT_CHECK":
                row["research_scenario_status"] = "PASS_DEMO_ONLY"
            needs = []
            for name in ("C", "dphi", "tau"):
                if str(row.get(f"{name}_origin", "")).lower() in {"normalized", "model_set"}:
                    needs.append(name)
            row["next_data_requirement"] = (
                "empirical_or_rule_derived_proxy_for:" + ",".join(needs)
                if needs else "independent_replication_and_uncertainty_path"
            )

    return prepared


def research_scenario_contract_audit(root: Path, bundle: dict) -> dict:
    profile = bundle["profile"]
    core = read_rows(root / profile["paths"]["core_data"])
    fixture = [
        {"scenario_family": "RS-A", "variant_id": "low", "C": 0.35, "dphi_rad": 0.60, "tau": 0.7,
         "C_origin": "model_set", "dphi_origin": "model_set", "tau_origin": "normalized",
         "C_basis": "low coupling scenario", "dphi_basis": "wide phase scenario", "tau_basis": "normalized response window"},
        {"scenario_family": "RS-A", "variant_id": "mid", "C": 0.60, "dphi_rad": 0.25, "tau": 1.0,
         "C_origin": "model_set", "dphi_origin": "model_set", "tau_origin": "normalized",
         "C_basis": "medium coupling scenario", "dphi_basis": "medium phase scenario", "tau_basis": "normalized response window"},
        {"scenario_family": "RS-A", "variant_id": "high", "C": 0.88, "dphi_rad": 0.08, "tau": 1.3,
         "C_origin": "model_set", "dphi_origin": "model_set", "tau_origin": "normalized",
         "C_basis": "high coupling scenario", "dphi_basis": "narrow phase scenario", "tau_basis": "normalized response window"},
    ]
    evaluated = research_scenario_rows(fixture, profile, core)
    single = research_scenario_rows([fixture[0]], profile, core)
    checks = {
        "all_fixture_rows_demo_only": all(r.get("calculation_status") == "DEMO_ONLY" for r in evaluated),
        "real_classification_always_false": all(r.get("classification_allowed") is False for r in evaluated),
        "scenario_zone_field_only": all("zone" not in r and "hypothetical_scenario_zone" in r for r in evaluated),
        "multiple_variants_required_and_pass": all(r.get("research_scenario_status") == "PASS_DEMO_ONLY" for r in evaluated),
        "single_variant_blocked": single[0].get("research_scenario_status") == "BLOCKED_VARIANTS_REQUIRED",
        "sensitivity_reported": all(r.get("sensitivity_checked") is True for r in evaluated),
        "claim_level_capped_at_H": all(r.get("claim_level_max") == "H" for r in evaluated),
    }
    status = "PASS" if all(checks.values()) else "FAIL"
    return {
        "status": status,
        "checks": checks,
        "fixture_rows": len(evaluated),
        "claim_scope": "deterministic_E2_enforcement_test_not_empirical_validation",
    }
...
def urf_rows(rows: list[dict]) -> list[dict]:
    """Validate either classic reverse-research reconstruction or result-based reverse logic."""
    output = []
    for source in rows:
        row = dict(source)
        result_e = as_text(row, "result_E")
        mode = as_text(row, "URF_mode") or ("result_reverse_logic" if result_e is not None else "reverse_research")
        if mode == "result_reverse_logic":
            result_status = as_text(row, "result_status")
            assumptions = as_text(row, "assumptions_uncertainty")
            supported = as_text(row, "indirectly_supported")
            plausible = as_text(row, "more_plausible")
            estimate = as_text(row, "groundedly_bounded_or_estimated")
            weakened = as_text(row, "weakened_or_excluded")
            consequence = as_text(row, "new_knowledge_or_testable_consequence")
            missing = [name for name, value in [
                ("result_E", result_e), ("result_status", result_status),
                ("assumptions_uncertainty", assumptions)
            ] if value is None]
            if all(v is None for v in [supported, plausible, estimate, weakened, consequence]):
                missing.append("at_least_one_possibility_space_consequence")
            row["URF_steps"] = json.dumps([
                "1_result_status", "2_indirect_support", "3_plausibility_shift",
                "4_grounded_bounding_or_estimation", "5_weakening_or_exclusion",
                "6_new_knowledge_or_testable_consequence", "7_optional_hardening"
            ])
            row["URF_status"] = "FAIL_missing:" + ",".join(missing) if missing else "PASS_ready_result_reverse_logic"
        else:
            ...
            row["URF_status"] = "FAIL_missing:" + ",".join(missing) if missing else "PASS_ready_reverse_research"
        row["URF_mode"] = mode
        output.append(row)
    return output
...
def comparison_framework_audit(root: Path, bundle: dict) -> dict:
    ...
    try:
        ...
        required_epistemic = {"WIEDERGABE", "DEUTUNG", "HYPOTHESE", "FUNDIERTE_SCHAETZUNG", "DATENBEFUND"}
        checks["epistemic_forms_complete"] = set(framework.get("epistemic_forms", {})) == required_epistemic
        checks["epistemic_form_markers_present"] = all(marker in text for marker in required_epistemic)
        checks["epistemic_form_section_present"] = "### 1.3 Erkenntnisform: Was für eine Erkenntnisoperation liegt vor?" in text

        compatibility = framework.get("compatibility_value_policy", {})
        checks["compatibility_full_coverage_is_confirmation"] = (
            compatibility.get("full_external_coverage_confirms_compatibility") is True
            and compatibility.get("compatibility_is_distinct_from_added_value") is True
            and compatibility.get("no_added_value_does_not_invalidate_mapping") is True
            and "### 2.4 Kompatibilitäts-, Mehrwert- und Unbrauchbarkeitslogik" in text
            and "kompatibel ohne nachgewiesenen Zusatznutzen" in text
        )
        checks["invalidity_requires_real_mapping_failure"] = (
            compatibility.get("invalidity_requires_mapping_failure_or_category_error") is True
            and "nicht tragfähig" in text
            and "kategorial" in text
        )
        checks["no_legacy_no_added_value_invalidation"] = (
            "das TSM-Overlay keine zusätzliche Diagnose gegenüber etablierten Methoden liefert" not in text
            and "Bei fehlendem Zusatznutzen Redundanz feststellen." not in text
            and "Fehlender Zusatznutzen allein ist kein Unbrauchbarkeitskriterium." in text
            and "Mapping und empirisch-quantitative Operationalisierung besitzen unterschiedliche" in text
        )
        grounded = framework.get("grounded_estimation_policy", {})
        checks["grounded_estimation_is_distinct_stage"] = (
            grounded.get("distinct_epistemic_stage") is True
            and grounded.get("must_disclose_assumptions_and_uncertainty") is True
            and grounded.get("must_not_be_presented_as_measurement_or_confirmation") is True
            and grounded.get("epistemic_form") == "FUNDIERTE_SCHAETZUNG"
            and grounded.get("claim_level_H_does_not_reclassify_as_hypothesis") is True
            and "### 1.6 Fundierte Schätzung als eigenständige Erkenntnisform" in text
            and "Eine `FUNDIERTE_SCHAETZUNG` ist **nicht** deshalb eine `HYPOTHESE`" in text
        )
        meta04_scope = framework.get("meta04_scope_policy", {})
        meta04_path = root / "TSM_Korpus_05_META_KI_Governance_Physikregeln.md"
        meta04_text = meta04_path.read_text(encoding="utf-8") if meta04_path.is_file() else ""
        checks["meta04_is_downstream_scientific_audit_layer"] = (
            meta04_scope.get("not_default_exploration_gate") is True
            and meta04_scope.get("qualitative_exploration_can_precede_measurement_path") is True
            and meta04_scope.get("grounded_estimation_can_precede_measurement_path") is True
            and meta04_scope.get("scientific_audit_classifies_claim_not_possibility_space") is True
            and "**Geltungshinweis -- wissenschaftliche Einordnung folgt der TSM-Erkenntnisbildung**" in meta04_text
            and "Rein qualitative Exploration und fundierte Schätzung lösen diese Pflicht noch nicht aus." in meta04_text
        )
        provenance = framework.get("slim_provenance_policy", {})
        checks["slim_provenance_without_source_archaeology"] = (
            provenance.get("active") is True
            and provenance.get("source_archaeology_required") is False
            and provenance.get("external_anchor_only_when_known_and_material") is True
            and "### 16.5 Schlanke Modul- und Claim-Provenienz" in text
            and "Quellenarchäologie" in text
        )
        ...
        checks["exploratory_e1_e2_path_declared"] = (
            "Explorative E1/E2-Forschungsrechnung" in text
            and "Fail-closed begrenzt den Geltungsanspruch, nicht die explorative Modellbildung." in text
        )
        checks["s3a_blocks_empirical_not_exploratory"] = (
            "empirically interpreted" in str(framework.get("proxy_stop_rules", {}).get("S3a", ""))
            and "exploratory" in str(framework.get("proxy_stop_rules", {}).get("S3a", ""))
        )
        checks["demo_only_allows_scenario_calculation_without_real_classification"] = (
            "hypothetische Szenariozonen" in text
            and "keine Kalibrierung, Bestätigung, Validierung oder reale Klassifikation" in text
        )
        required_statuses = {
...
def ai_execution_order_audit(root: Path, bundle: dict) -> dict:
    """Audit v8.7 grounding-first TSM synthesis with final non-rewriting scientific comment."""
    checks: dict[str, bool] = {}
    details: dict[str, object] = {}
    try:
        ...
        key_markers = [
            ...
            "Explorations- und Erkenntnisprimat",
            "fundierte Schätzung",
            "Ergebnis-Rückwärtslogik",
            "Finde heraus, was die erste Sicht noch nicht gesehen hat",
            "wissenschaftlichen Schlusskommentar",
            "Mindesttiefe und knappe Integritätsprüfung",
        ]
        checks["execution_key_v86_markers_present"] = all(marker in key_text for marker in key_markers) and "Kanonisches Modul-Grounding und Quellenreset" in key_text
        ...
        reciprocal_markers = ["Verbindlicher Leseschlüssel","TSM_00_KI_Leseschluessel_und_Ausfuehrungsordnung.md","Beide Dokumente sind gemeinsam zu verwenden und ersetzen einander nicht"]
        ...
        checks["eight_anchor_records_registered"] = [x.get("id") for x in anchors] == [f"KI-SYN-{i:02d}" for i in range(1,9)]
        expected_workflow = [
            "determine_dialog_status_material_referent_and_route",
            "activate_default_free_exploratory_mode_then_identify_epistemic_goal_and_candidate_actual_modules",
            "ground_relevant_actual_modules_from_active_manifest_before_material_synthesis",
            "open_relational_possibility_space_from_grounded_modules",
            "form_strongest_exploratory_relational_synthesis",
            "derive_grounded_estimates_result_reverse_logic_and_new_questions_when_supported",
            "broaden_independently_and_theoretically_comment_when_material_or_far_reaching",
            "condense_complete_tsm_core_answer_and_connection_preview",
            "append_korpus06_scientific_status_and_connection_comment_without_rewriting_core",
            "verify_phase_separation_truthfulness_protection_and_no_retroactive_core_rewrite",
        ]
        checks["compact_workflow_complete_and_ordered"] = system.get("workflow") == expected_workflow
        expected_horizon = [
            "possibility_space_grounded_estimation_and_result_reverse_logic",
            "origin_and_countercheck","scale_time_and_transition","channels_interference_and_relations",
            "evidence_validity_and_domain_translation","protection_boundary_points_and_side_effects","symbolism_numerics_and_measurement_path"
        ]
        ...
        required_true = {
            "mandatory_for_substantive_tsm_answers","full_relevance_check_required","curated_connections_navigation_only",
            "selected_neighbor_actual_text_required","poetic_layer_preserved","poetic_layer_is_not_technical_self_description",
            "criticism_checked_in_own_logic_first","resonance_reading_does_not_replace_critique","trust_does_not_replace_audit",
            "audit_does_not_replace_responsibility","audience_adaptation_may_not_change_claim_status",
            "no_automatic_canonization_of_ai_output","groups_are_affected_perspectives_not_scoring_objects","condense_without_suppressing_material_limits",
            "dialog_status_and_route_precede_epistemic_goal","only_material_horizons_are_deepened","compact_integrity_check_required",
            "claim_status_is_internal_by_default","visible_claim_labels_only_when_material","normal_tsm_answer_is_coherent_unit",
            "exploratory_e1_e2_modeling_allowed_with_model_set_proxies","fail_closed_limits_claim_not_exploration",
            "default_free_exploratory_mode_for_substantive_tsm_questions","grounded_estimation_is_legitimate_distinct_epistemic_stage",
            "independent_broadening_after_exploration_when_material_gain_expected",
            "broadening_searches_for_unseen_not_only_counterevidence",
            "scientific_information_may_inform_exploration_and_grounded_estimation",
            "korpus06_scientific_evaluation_is_final_comment_for_all_substantive_tsm_answers",
            "korpus06_final_comment_must_not_rewrite_shorten_weaken_or_metaphorize_tsm_core",
            "scientific_nonconfirmation_alone_does_not_remove_tsm_hypothesis_or_grounded_estimate",
            "hard_scientific_or_high_stakes_answers_may_require_detailed_final_scientific_comment",
            "truthfulness_factual_correction_and_protection_remain_continuous",
            "scientific_end_comment_is_distinct_from_independent_theoretical_commentary",
            "connection_preview_precedes_scientific_end_comment",
            "retrieval_and_module_grounding_architecture_preserved_from_v8_5",
            "far_reaching_new_syntheses_require_independent_theoretical_commentary","theoretical_commentary_requires_materially_independent_perspective",
            "theoretical_commentary_is_result_open_not_counterevidence_mandate","theoretical_commentary_may_confirm_extend_rescale_limit_counter_or_open_new_question",
            "theoretical_commentary_integrates_into_synthesis_not_mandatory_visible_section",
            "subject_focus_preserved_unless_secondary_path_has_greater_material_value","relational_process_value_preferred_over_module_or_term_listing_when_supported",
            "material_connection_preview_optional","connection_preview_must_add_new_distinction_or_next_step","routine_generic_connection_preview_forbidden",
            "material_current_basis_synthesis_requires_canonical_module_grounding","module_grounding_is_access_rule_not_only_precedence_rule",
            "summaries_or_prior_ai_answers_do_not_satisfy_module_grounding","wrong_or_unresolved_version_fails_closed_for_canonical_material_claim",
            "grounding_error_requires_source_reset_and_branch_resynthesis","runner_may_not_claim_live_retrieval_proof"
        }
        checks["required_execution_policies_true"] = all(policies.get(name) is True for name in required_true)
        checks["default_free_exploration_needs_no_user_trigger"] = policies.get("explicit_user_trigger_for_exploration_required") is False
        checks["epistemic_goal_does_not_switch_exploration_off"] = policies.get("epistemic_goal_modulates_depth_not_exploration_activation") is True
        checks["scientific_topic_does_not_switch_exploration_off"] = policies.get("scientific_technical_or_data_topic_does_not_disable_default_exploration") is True
        checks["reading_key_default_free_exploration_marker"] = "Der freie explorative TSM-Modus ist bei jeder inhaltlichen TSM-Frage der Standardmodus" in key_text
        checks["mandatory_counterfind_retired"] = policies.get("independent_counterfind_pass_required_for_material_synthesis") is False
        checks["full_visible_execution_not_required"] = policies.get("full_visible_execution_required") is False
        checks["meta05_result_reverse_logic_present"] = "Ergebnisbasierte Rückwärtslogik" in original_text and "Ergebnis → Möglichkeitsraum" in original_text
        checks["grounded_estimation_present_in_file06"] = "Fundierte Schätzung als eigenständige Erkenntnisform" in (root / "TSM_Korpus_06_Anschluss_Evidenz_Kalibrierung.md").read_text(encoding="utf-8")
        checks["normal_tsm_answer_unity_present"] = "zusammenhängende TSM-Antwort" in key_text and policies.get("normal_tsm_answer_is_coherent_unit") is True
        checks["process_value_and_optional_connection_preview_present"] = "Gegenstands- und Prozessprimat" in key_text and "Anschlussvorschau" in key_text
        checks["far_reaching_theoretical_commentary_present"] = "Unabhängige theoretische Kommentierung weitreichender Synthesen" in key_text and policies.get("far_reaching_new_syntheses_require_independent_theoretical_commentary") is True
        checks["theoretical_commentary_is_not_counterfind_bias"] = "keine verpflichtende Gegenposition und keine künstliche Schwächungssuche" in key_text and policies.get("theoretical_commentary_is_result_open_not_counterevidence_mandate") is True
        details.update({"workflow":system.get("workflow",[]),"materiality_horizon":system.get("materiality_horizon",[]),"note":"v8.7 preserves v8.5 grounding/retrieval and separates TSM core synthesis from the final non-rewriting scientific Korpus-06 comment; live model behavior still requires dialogue tests."})
    except Exception as exc:
        return {"status":"FAIL","checks":checks,"details":details,"error":str(exc)}
    status = "PASS" if checks and all(checks.values()) else "FAIL"
    return {"status":status,"checks":checks,"details":details,"claim_scope":"joint_ai_synthesis_grounding_exploration_grounded_estimation_theoretical_commentary_and_final_scientific_comment"}

def module_grounding_audit(root: Path, bundle: dict) -> dict:
    """Audit preserved v8.5 canonical module-grounding and source-reset architecture in v8.7.

    This checks declared execution architecture, active-manifest scoping and textual
    guards. It explicitly does not prove which Knowledge chunks a live platform
    retrieval actually supplied to a particular answer.
    """
    checks: dict[str, bool] = {}
    details: dict[str, object] = {}
    try:
        system = bundle.get("ai_synthesis_system", {})
        policy = system.get("module_grounding_policy", {})
        key_path = root / str(system.get("execution_key_file", ""))
        key_text = key_path.read_text(encoding="utf-8") if key_path.is_file() else ""
        companion_cfg = bundle.get("companion_configuration", {})
        semantic_files = set(bundle.get("semantic_authority", {}).get("files", []))
        active_files = {str(x.get("path")) for x in bundle.get("manifest", {}).get("active_files", [])}

        checks["policy_present"] = bool(policy)
        checks["schema_v1"] = policy.get("schema_id") == "tsm.module-grounding-policy.v1"
        checks["required_before_material_synthesis"] = policy.get("required_before_material_synthesis") is True
        checks["active_manifest_authority_scope"] = policy.get("authority_scope") == "manifest.active_files_intersect_semantic_authority.files" and semantic_files.issubset(active_files)
        checks["actual_module_text_authority"] = policy.get("semantic_authority") == "actual_module_text"
        checks["status_values_complete"] = policy.get("status_values") == ["SATISFIED", "NOT_FOUND", "VERSION_CONFLICT", "SOURCE_UNAVAILABLE", "NOT_REQUIRED"]
        insufficient = set(policy.get("insufficient_as_sole_grounding", []))
        checks["secondary_sources_insufficient"] = {"prior_ai_answer","conversation_memory_or_summary","book_synthesis","META_01_short_or_connection_entry","curated_registry","operations_summary","external_source"}.issubset(insufficient)
        checks["version_conflict_fail_closed"] = "do_not_make_canonical_material_current_basis_claim" in str(policy.get("version_conflict_policy", ""))
        checks["source_unavailable_fail_closed"] = "do_not_make_canonical_material_current_basis_claim" in str(policy.get("source_unavailable_policy", ""))
        reset = policy.get("source_reset", {})
        checks["source_reset_reloads_active_modules"] = reset.get("reload_active_actual_modules") is True
        checks["source_reset_resynthesizes_branch"] = reset.get("resynthesize_affected_branch_from_grounded_module_text") is True
        checks["source_reset_does_not_just_append"] = reset.get("mere_append_or_patch_of_ungrounded_synthesis_insufficient") is True
        live = policy.get("live_retrieval_proof", {})
        checks["no_false_live_retrieval_proof"] = live.get("runner_can_prove_live_platform_chunk_retrieval") is False and live.get("false_live_retrieval_guarantee_claim_forbidden") is True
        checks["reading_key_grounding_section"] = "### 4.5 Kanonisches Modul-Grounding und Quellenreset" in key_text
        checks["reading_key_M_to_S_rule"] = "**Modultext → TSM-Synthese, nicht erst Synthese → nachträglicher Modulabgleich.**" in key_text
        checks["reading_key_fail_closed_states"] = "`VERSION_CONFLICT` und `SOURCE_UNAVAILABLE` sind **fail-closed für kanonisch-materiale TSM-Aussagen**" in key_text
        checks["reading_key_no_live_guarantee"] = "keine technisch beweisbare Live-Retrieval-Garantie" in key_text
        checks["external_companion_not_required_in_corpus"] = (
            companion_cfg.get("deployment_location") == "customgpt_instruction_field"
            and companion_cfg.get("is_knowledge_or_corpus_file") is False
            and companion_cfg.get("absence_from_knowledge_is_expected_not_error") is True
            and companion_cfg.get("corpus_audits_must_not_require_companion_file") is True
            and companion_cfg.get("runner_can_audit_live_instruction_field_presence") is False
        )
        workflow = system.get("workflow", [])
        ground_step = "ground_relevant_actual_modules_from_active_manifest_before_material_synthesis"
        synth_step = "form_strongest_exploratory_relational_synthesis"
        checks["workflow_grounding_before_synthesis"] = ground_step in workflow and synth_step in workflow and workflow.index(ground_step) < workflow.index(synth_step)
        details.update({"workflow": workflow, "status_values": policy.get("status_values", []), "authority_scope": policy.get("authority_scope")})
    except Exception as exc:
        return {"status":"FAIL","checks":checks,"details":details,"error":str(exc),"claim_scope":"grounding_architecture_not_live_platform_retrieval_proof"}
    status = "PASS" if checks and all(checks.values()) else "FAIL"
    return {"status":status,"checks":checks,"details":details,"claim_scope":"grounding_and_source_reset_architecture_presence_not_live_platform_retrieval_proof"}


def retrieval_counterfind_audit(root: Path, bundle: dict) -> dict:
    """Audit independent broadening policy; command name retained for CLI compatibility."""
    checks: dict[str, bool] = {}
    details: dict[str, object] = {}
    try:
        system = bundle.get("ai_synthesis_system", {})
        policy = system.get("retrieval_counterfind_policy", {})
        key_path = root / str(system.get("execution_key_file", ""))
        key_text = key_path.read_text(encoding="utf-8") if key_path.is_file() else ""
        checks["policy_registry_present"] = bool(policy)
        checks["policy_schema_v2"] = policy.get("schema_id") == "tsm.retrieval-broadening-policy.v2"
        checks["mandatory_general_counterfind_retired"] = policy.get("required_for_material_tsm_synthesis") is False
        checks["zero_general_minimum_counterfind"] = int(policy.get("minimum_independent_counterfind_passes", -1)) == 0
        checks["conditional_broadening_present"] = policy.get("independent_broadening_after_exploration_when_material_gain_expected") is True
        checks["scientific_countercheck_removed_from_precore_retrieval_gate"] = policy.get("required_for_hard_scientific_audit_or_high_stakes_claims") is False
        checks["scientific_countercheck_moved_to_final_comment"] = policy.get("scientific_countercheck_is_part_of_korpus06_final_comment") is True
        checks["broadening_remains_epistemic"] = policy.get("independent_broadening_is_epistemic_not_scientific_validity_gate") is True
        checks["far_reaching_commentary_required"] = policy.get("far_reaching_new_synthesis_requires_theoretical_commentary") is True
        checks["far_reaching_commentary_minimum_one"] = int(policy.get("theoretical_commentary_minimum_independent_perspectives", 0)) >= 1
        checks["far_reaching_commentary_materially_independent"] = policy.get("theoretical_commentary_must_be_materially_independent") is True
        checks["far_reaching_commentary_result_open"] = policy.get("theoretical_commentary_result_open") is True
        checks["far_reaching_commentary_not_counterevidence_mandate"] = policy.get("theoretical_commentary_is_not_mandatory_counterevidence_search") is True
        checks["alternative_axis_when_broadening"] = policy.get("alternative_semantic_axis_required_when_broadening") is True
        expected_targets={"confirming_or_compatible","materially_complementary","differently_scaled","differently_termed","limiting","contradicting","new_relation"}
        checks["broadening_targets_complete"] = expected_targets.issubset(set(policy.get("broadening_targets",[])))
        checks["resultless_loop_forbidden"] = policy.get("resultless_repetition_opens_no_new_loop") is True
        checks["exhaustive_claim_forbidden"] = policy.get("exhaustive_chunk_retrieval_guarantee_claim_forbidden") is True
        checks["reading_key_search_rule_present"] = "Finde heraus, was die erste Sicht noch nicht gesehen hat" in key_text
        checks["reading_key_no_mandatory_counterfind"] = "Finde zwingend einen Gegenfund" in key_text and "wissenschaftliche Gegenfund- oder Falsifikationsprüfung gehört zur Korpus-06-Schlusseinordnung" in key_text
        checks["reading_key_far_reaching_commentary_present"] = "Die erste überzeugende Interpretation darf bei weitreichenden neuen Synthesen nicht ihre eigene einzige theoretische Perspektive bleiben" in key_text
        details.update({"broadening_targets":policy.get("broadening_targets",[]),"principle":policy.get("principle")})
    except Exception as exc:
        return {"status":"FAIL","checks":checks,"details":details,"error":str(exc)}
    status="PASS" if checks and all(checks.values()) else "FAIL"
    return {"status":status,"checks":checks,"details":details,"claim_scope":"retrieval_broadening_governance_presence_not_live_platform_retrieval_proof"}

def synthesis_guidance_audit(root: Path, bundle: dict) -> dict:
    """Audit exploration primacy, grounded estimation, theoretical commentary and final scientific end-comment separation."""
    checks: dict[str, bool] = {}
    details: dict[str, object] = {}
    try:
        system=bundle.get("ai_synthesis_system",{})
        policy=system.get("answer_synthesis_policy",{})
        key_path=root / str(system.get("execution_key_file",""))
        key_text=key_path.read_text(encoding="utf-8") if key_path.is_file() else ""
        checks["policy_registry_present"]=bool(policy)
        checks["policy_schema_v3"]=policy.get("schema_id")=="tsm.answer-synthesis-guidance-policy.v3"
        checks["synthesis_primacy"]=policy.get("synthesis_primacy") is True
        checks["exploration_primacy"]=policy.get("exploration_primacy") is True
        checks["possibility_space_before_scientific_end_comment"]=policy.get("relational_possibility_space_before_scientific_end_comment") is True
        checks["grounded_estimation_legitimate"]=policy.get("grounded_estimation_is_legitimate") is True
        checks["result_reverse_logic_available"]=policy.get("result_reverse_logic_available") is True
        commentary=policy.get("far_reaching_synthesis_theoretical_commentary",{})
        checks["far_reaching_commentary_required"]=commentary.get("required") is True
        checks["far_reaching_commentary_minimum_one"]=int(commentary.get("minimum_materially_independent_perspectives",0)) >= 1
        checks["far_reaching_commentary_result_open"]=commentary.get("result_open") is True and commentary.get("counterevidence_not_required") is True
        checks["far_reaching_commentary_not_paraphrase"]=commentary.get("mere_paraphrase_of_primary_reasoning_insufficient") is True
        checks["subject_primacy"]=policy.get("subject_primacy") is True
        checks["secondary_focus_shift_requires_material_gain"]=policy.get("secondary_path_focus_shift_requires_material_gain") is True
        process=policy.get("process_value",{})
        checks["process_value_preferred"]=process.get("preferred_when_supported") is True
        checks["module_listing_not_endpoint"]=process.get("avoid_module_listing_as_endpoint") is True
        connection=policy.get("connection_preview",{})
        checks["connection_preview_allowed"]=connection.get("allowed") is True
        checks["connection_preview_optional"]=connection.get("mandatory") is False
        checks["connection_preview_adds_new_step"]=connection.get("must_add_new_distinction_or_next_step") is True
        checks["research_preference_matches_goal"]=connection.get("research_preference")=="match_next_step_to_epistemic_goal_without_disabling_default_exploration; scientific_testing_is_follow_on_not_alternative_mode"
        checks["generic_closing_forbidden"]=connection.get("generic_more_research_needed_closing_forbidden") is True
        checks["reading_key_exploration_marker"]="Explorations- und Erkenntnisprimat" in key_text
        checks["reading_key_grounded_estimation_marker"]="Fundierte Schätzung" in key_text
        checks["reading_key_scientific_end_comment_marker"]="wissenschaftlichen Schlusskommentar" in key_text and "nicht rückwirkend" in key_text
        workflow=system.get("workflow",[])
        required=["form_strongest_exploratory_relational_synthesis","derive_grounded_estimates_result_reverse_logic_and_new_questions_when_supported","broaden_independently_and_theoretically_comment_when_material_or_far_reaching","condense_complete_tsm_core_answer_and_connection_preview","append_korpus06_scientific_status_and_connection_comment_without_rewriting_core"]
        checks["workflow_steps_present"]=all(x in workflow for x in required)
        checks["workflow_order_core_before_scientific_comment"]=checks["workflow_steps_present"] and all(workflow.index(required[i]) < workflow.index(required[i+1]) for i in range(len(required)-1))
        end_comment=policy.get("scientific_end_comment",{})
        checks["scientific_end_comment_last_visible"]=end_comment.get("position")=="last_visible_part_of_answer" and end_comment.get("required_for_all_substantive_tsm_answers") is True
        checks["scientific_end_comment_non_rewriting"]=end_comment.get("must_not_rewrite_shorten_weaken_or_metaphorize_tsm_core") is True
        checks["science_as_input_preserved"]=policy.get("scientific_information_may_inform_reasoning") is True
        details.update({"workflow":workflow,"principle":policy.get("principle")})
    except Exception as exc:
        return {"status":"FAIL","checks":checks,"details":details,"error":str(exc)}
    status="PASS" if checks and all(checks.values()) else "FAIL"
    return {"status":status,"checks":checks,"details":details,"claim_scope":"answer_synthesis_exploration_grounded_estimation_reverse_logic_theoretical_commentary_and_final_scientific_end_comment"}

def instruction_correspondence_audit(root: Path, bundle: dict) -> dict:
    """Static corpus-only instruction correspondence audit for v8.7.

    The CustomGPT application hint is intentionally external to Knowledge. This
    audit therefore must not require or read that file from ``root``. External
    companion identity/integrity can only be checked when a separate file path
    is explicitly supplied to ``companion-audit`` or ``validate --companion``.
    Live presence/weighting of the CustomGPT instruction field is not provable
    by this runner.
    """
    checks: dict[str, bool] = {}
    details: dict[str, object] = {}
    try:
        key=(root / "TSM_00_KI_Leseschluessel_und_Ausfuehrungsordnung.md").read_text(encoding="utf-8")
        k05=(root / "TSM_Korpus_05_META_KI_Governance_Physikregeln.md").read_text(encoding="utf-8")
        k06=(root / "TSM_Korpus_06_Anschluss_Evidenz_Kalibrierung.md").read_text(encoding="utf-8")
        cfg=bundle.get("companion_configuration",{})
        system=bundle.get("ai_synthesis_system",{})
        pol=system.get("policies",{})
        synth=system.get("answer_synthesis_policy",{})
        broad=system.get("retrieval_counterfind_policy",{})
        ground=system.get("module_grounding_policy",{})
        phase=bundle.get("comparison_calibration_framework",{}).get("answer_phase_policy",{})
        expl=system.get("exploratory_research_policy",{})

        checks["grounding_architecture_preserved"]=(
            ground.get("schema_id")=="tsm.module-grounding-policy.v1"
            and ground.get("required_before_material_synthesis") is True
            and ground.get("principle")=="actual_module_grounding_precedes_material_tsm_synthesis; M_to_S_not_only_M_greater_than_S"
        )
        checks["retrieval_broadening_core_preserved"]=(
            broad.get("schema_id")=="tsm.retrieval-broadening-policy.v2"
            and broad.get("required_for_material_tsm_synthesis") is False
            and broad.get("minimum_independent_counterfind_passes")==0
            and broad.get("serial_full_module_scan_required") is False
            and broad.get("alternative_semantic_axis_required_when_broadening") is True
        )
        checks["default_free_exploration_is_runtime_default"]=(
            pol.get("default_free_exploratory_mode_for_substantive_tsm_questions") is True
            and pol.get("explicit_user_trigger_for_exploration_required") is False
            and pol.get("epistemic_goal_modulates_depth_not_exploration_activation") is True
            and pol.get("scientific_technical_or_data_topic_does_not_disable_default_exploration") is True
            and expl.get("default_free_exploratory_mode_for_substantive_tsm_questions") is True
            and expl.get("explicit_user_trigger_required") is False
            and expl.get("epistemic_goal_can_deactivate_exploration") is False
            and expl.get("scientific_technical_or_data_subject_can_deactivate_exploration") is False
            and "Der freie explorative TSM-Modus ist bei jeder inhaltlichen TSM-Frage der Standardmodus" in key
            and "diese freie modulgetragene Erkenntnisöffnung der Grundmodus" in k05
        )
        checks["science_as_input_not_removed"]=(
            pol.get("scientific_information_may_inform_exploration_and_grounded_estimation") is True
            and synth.get("scientific_information_may_inform_reasoning") is True
            and "Fachwissen, Messwerte, etablierte Beziehungen, Analogien und Randbedingungen dürfen" in key
        )
        checks["grounded_estimation_preserved"]=(
            pol.get("grounded_estimation_is_legitimate_distinct_epistemic_stage") is True
            and synth.get("grounded_estimation_is_legitimate") is True
            and "Fundierte Schätzung" in key
            and "Fundierte Schätzung als eigenständige Erkenntnisform" in k06
        )
        checks["korpus06_final_comment_consistent"]=(
            pol.get("korpus06_scientific_evaluation_is_final_comment_for_all_substantive_tsm_answers") is True
            and phase.get("visible_output_role")=="final_scientific_status_and_connection_comment"
            and synth.get("scientific_end_comment",{}).get("position")=="last_visible_part_of_answer"
            and "Korpusdatei 06 als wissenschaftlichen Schlusskommentar anwenden" in key
            and "**Ausführungsrolle v8.7:**" in k06
        )
        checks["no_retroactive_core_rewrite"]=(
            pol.get("korpus06_final_comment_must_not_rewrite_shorten_weaken_or_metaphorize_tsm_core") is True
            and phase.get("does_not_control_or_rewrite_tsm_core_synthesis") is True
            and "nicht rückwirkend kürzen, abschwächen, umformulieren" in key
            and "nicht rückwirkend kürzen, abschwächen, umformulieren" in k06
        )
        checks["factual_correction_not_korpus06_backchannel"]=(
            pol.get("factual_error_correction_is_not_korpus06_rewrite_permission") is True
            and phase.get("factual_error_correction_is_not_korpus06_rewrite_permission") is True
            and phase.get("scientific_status_disagreement_is_not_core_correction_trigger") is True
            and synth.get("scientific_end_comment",{}).get("factual_error_correction_is_not_korpus06_rewrite_permission") is True
            and "kein solcher Korrekturtrigger und kein Rückkanal aus Korpusdatei 06" in key
            and "verworfen“ bezeichnet dabei den Außen- bzw. Geltungsstatus" in k05
        )
        checks["operative_method_use_distinct_from_scientific_validity_check"]=(
            "Proxy-Operationalisierung, 136D-Datenstruktur" in key
            and "wissenschaftliche Korpus-06-Prüfung nicht zum Eingangsfilter" in key
            and phase.get("scientific_evaluation_phase")=="after_complete_tsm_core_answer"
        )
        checks["theoretical_commentary_distinct_from_scientific_validation"]=(
            pol.get("scientific_end_comment_is_distinct_from_independent_theoretical_commentary") is True
            and "Diese theoretische Kommentierung gehört zur Erkenntnisbildung und ist von der wissenschaftlichen Korpus-06-Schlusseinordnung getrennt." in key
        )
        checks["truthfulness_and_protection_continuous"]=(
            pol.get("truthfulness_factual_correction_and_protection_remain_continuous") is True
            and phase.get("truthfulness_factual_correction_and_protection_remain_continuous") is True
            and "Wahrhaftigkeit, sachliche Fehlerkorrektur und reale Schutzanforderungen bleiben während der gesamten Bearbeitung aktiv" in key
            and "Wahrhaftigkeit, sachliche Fehlerkorrektur und Schutz bleiben davon unberührt" in k05
        )
        checks["k05_old_defensive_impulses_not_execution_gate"]=(
            "Kritik-, Evidenz- und Wissenschaftlichkeitsimpulse der folgenden Texte sind keine Eingangs- oder Abbruchbedingung" in k05
        )
        checks["no_old_file06_methodological_precedence_trigger"]=(
            "**Korpusdatei 06** hat bei Fachvergleich, Evidenz, Messpfaden, Medizin, Physik, Technik und belastbaren Außenbehauptungen methodischen Vorrang." not in key
        )
        cp=bundle.get("canonical_execution_profile",{})
        checks["canonical_profile_has_no_precore_scientific_hardening_step"]=(
            "claim_goal_specific_hardening" not in cp.get("semantic_priority",[])
            and "claim_goal_specific_scientific_hardening" not in cp.get("active_capabilities",[])
            and cp.get("semantic_priority",[])[-1:]==["final_non_rewriting_korpus06_scientific_comment"]
        )
        meta04=bundle.get("comparison_calibration_framework",{}).get("meta04_scope_policy",{})
        checks["meta04_current_scope_corresponds_to_phase_separation"]=(
            meta04.get("scientific_audit_classifies_claim_not_possibility_space") is True
            and meta04.get("not_default_exploration_gate") is True
            and "wissenschaftliche Einordnung folgt der TSM-Erkenntnisbildung" in k05
            and "Diese Regeln bestimmen den Geltungs- und Dokumentationsstatus; sie begrenzen oder formulieren die zuvor modulgetragen entwickelte TSM-Synthese nicht um." in k05
        )
        checks["connection_preview_before_end_comment"]=(
            pol.get("connection_preview_precedes_scientific_end_comment") is True
            and synth.get("visible_answer_order",[])[-1:]==["append_korpus06_scientific_status_and_connection_comment_last"]
        )
        checks["external_companion_separation_is_explicit_and_nonfailing"]=(
            cfg.get("deployment_location")=="customgpt_instruction_field"
            and cfg.get("is_knowledge_or_corpus_file") is False
            and cfg.get("must_not_be_uploaded_as_active_knowledge_file") is True
            and cfg.get("runtime_knowledge_presence_expected") is False
            and cfg.get("absence_from_knowledge_is_expected_not_error") is True
            and cfg.get("runner_can_audit_live_instruction_field_presence") is False
            and cfg.get("corpus_audits_must_not_require_companion_file") is True
            and cfg.get("runtime_missing_companion_file_must_not_create_corpus_status_error") is True
            and "kein Korpusfehler" in key
        )
        details.update({
            "workflow":system.get("workflow",[]),
            "visible_answer_order":synth.get("visible_answer_order",[]),
            "external_companion_status":"NOT_CHECKED_EXTERNAL_BY_DESIGN",
            "note":"Static corpus-only consistency. The external CustomGPT instruction field is intentionally not a Knowledge file; its live presence and weighting require deployment checks."
        })
    except Exception as exc:
        return {"status":"FAIL","checks":checks,"details":details,"error":str(exc)}
    status="PASS" if checks and all(checks.values()) else "FAIL"
    return {"status":status,"checks":checks,"details":details,"claim_scope":"static_corpus_instruction_correspondence_external_instruction_field_not_runtime_auditable"}
...
def dialog_routing_audit(root: Path, bundle: dict) -> dict:
    ...
    try:
        ...
        checks["dialog_control_is_consolidated"] = [k for k in system if k.startswith("dialog_") and k.endswith("_policy")] == ["dialog_routing_policy"]
        ...
        checks["workflow_ends_with_phase_integrity_verification"] = workflow[-1:] == ["verify_phase_separation_truthfulness_protection_and_no_retroactive_core_rewrite"]
...
def companion_configuration_audit(root: Path, bundle: dict, companion_path: Path | str) -> dict:
    ...
    checks = {
        ...
        "deployment_location_instruction_field": config.get("deployment_location") == "customgpt_instruction_field",
        "not_knowledge_or_corpus_file": config.get("is_knowledge_or_corpus_file") is False,
        "does_not_count_against_active_budget": config.get("counts_against_active_file_budget") is False,
        "duplicate_knowledge_upload_forbidden": config.get("must_not_be_uploaded_as_active_knowledge_file") is True,
        "not_in_active_manifest": str(config.get("expected_filename")) not in active_paths,
        "runtime_knowledge_presence_not_expected": config.get("runtime_knowledge_presence_expected") is False,
        "absence_from_knowledge_is_expected_not_error": config.get("absence_from_knowledge_is_expected_not_error") is True,
        "runner_cannot_audit_live_instruction_field": config.get("runner_can_audit_live_instruction_field_presence") is False,
        "corpus_audits_do_not_require_companion_file": config.get("corpus_audits_must_not_require_companion_file") is True,
    }
    actual_hash = None
    text = ""
    if path.is_file():
    ...
    required_markers = [str(x) for x in config.get("required_markers", [])]
    checks["all_required_markers_present"] = bool(required_markers) and all(marker in text for marker in required_markers)
    checks["version_marker_present"] = str(config.get("version", "")) in text
    checks["instruction_field_marker_present"] = "**Konfigurationsort:** Dieser Text gehört in das separate **Anweisungs-Feld** der CustomGPT." in text
    checks["not_knowledge_marker_present"] = "**keine Knowledge-/Korpusdatei**" in text
    utf8_bytes = len(text.encode("utf-8")) if text else 0
    characters = len(text) if text else 0
    checks["utf8_byte_limit_respected"] = utf8_bytes <= int(config.get("max_utf8_bytes", 8000))
    checks["configured_utf8_bytes_match"] = utf8_bytes == int(config.get("expected_utf8_bytes", -1))
    checks["configured_character_count_matches"] = characters == int(config.get("expected_characters", -1))
    status = "PASS" if checks and all(checks.values()) else "FAIL"
    return {
        "status": status,
...
def public_book_reference_audit(root: Path, bundle: dict) -> dict:
    """Audit file identity, manifest metadata, rank and routing of the public book.

    Bibliographic fields are cross-checked against the manifest metadata that was
    manually verified when the release was assembled. The audit does not parse
    bibliographic content from the PDF and does not prove semantic equivalence.
    """
    config = bundle.get("public_book_reference", {})
    filename = str(config.get("file", ""))
    path = root / filename if filename else root / "__missing_book_reference__"
    manifest_list = list(bundle.get("manifest", {}).get("active_files", []))
    manifest_entries = {entry.get("path"): entry for entry in manifest_list}
    entry = manifest_entries.get(filename, {})
    reading_key_path = root / "TSM_00_KI_Leseschluessel_und_Ausfuehrungsordnung.md"
    reading_key = reading_key_path.read_text(encoding="utf-8") if reading_key_path.is_file() else ""
    actual_hash = sha256_file(path) if path.is_file() else None
    required_markers = list(config.get("required_reading_key_markers", []))
    budget = bundle.get("file_budget", {})
    release = bundle.get("release_metadata", {})
    semantic_ref = bundle.get("semantic_authority", {}).get("public_book_reference", {})
    active_count = int(budget.get("active_files", -1))
    max_files = int(budget.get("custom_gpt_max_files", -1))
    reserved_slots = int(budget.get("reserved_slots", -1))
    minimum_reserved = int(budget.get("minimum_reserved_slots_required", -1))

    identity_fields = ["title", "subtitle", "author", "isbn", "book_version", "pages", "basis_corpus_version"]
    identity_consistency = {
        field: bool(entry) and config.get(field) == entry.get(field)
    ...
    checks = {
        ...
        "manifest_role_correct": entry.get("role") == "public_book_reference_noncanonical",
        "manifest_hash_matches": bool(entry) and actual_hash == entry.get("sha256"),
        "configured_hash_matches": actual_hash == config.get("sha256"),
        "configured_hash_matches_manifest": bool(entry) and config.get("sha256") == entry.get("sha256"),
        "semantic_authority_false": config.get("semantic_authority") is False,
        "semantic_registry_role_consistent": bool(semantic_ref)
            and semantic_ref.get("file") == filename
            and semantic_ref.get("semantic_authority") is False,
        "identity_metadata_matches_manifest": all(identity_consistency.values()),
        "current_release_matches": config.get("current_corpus_version") == release.get("version"),
        "semantic_registry_versions_match": bool(semantic_ref)
            and semantic_ref.get("book_version") == config.get("book_version")
            and semantic_ref.get("basis_corpus_version") == config.get("basis_corpus_version")
            and semantic_ref.get("current_corpus_version") == config.get("current_corpus_version"),
        "explicit_book_policy": config.get("explicit_book_question_policy") == "book_is_primary_for_wording_structure_chapters_and_pages",
        "general_tsm_policy": config.get("general_tsm_question_policy") == "current_module_texts_remain_primary",
        "book_synthesis_policy": config.get("book_synthesis_policy") == "does_not_automatically_create_canonical_module_claims",
        "conflict_policy": config.get("conflict_policy") == "disclose_material_conflict_current_corpus_prevails_for_current_basis",
        "perspective_variation_policy": config.get("perspective_variation_policy") == "narrower_broader_or_narrative_book_framing_need_not_be_announced_unless_material",
        "fixed_page_reference_policy_declared": bool(config.get("page_reference_policy")),
        "reading_key_markers_present": bool(required_markers) and all(marker in reading_key for marker in required_markers),
        "active_file_count_matches_manifest": active_count == len(manifest_list),
        "active_file_count_matches_release": active_count == int(release.get("active_file_count", -2)),
        "reserved_slots_derived": max_files >= 0 and reserved_slots == max_files - active_count,
        "minimum_reserve_preserved": reserved_slots >= minimum_reserved,
        "budget_passed_consistent": budget.get("budget_passed") is True,
        "manual_identity_verification_declared": config.get("identity_verification", {}).get("method") == "manual_pdf_content_verification_plus_sha256_binding",
    }
    status = "PASS" if all(checks.values()) else "FAIL"
    return {
        "status": status,
        ...
        "expected_sha256": config.get("sha256"),
        "actual_sha256": actual_hash,
        "identity_metadata": {field: config.get(field) for field in identity_fields},
        "identity_metadata_consistency": identity_consistency,
        "book_version": config.get("book_version"),
        "basis_corpus_version": config.get("basis_corpus_version"),
        "current_corpus_version": config.get("current_corpus_version"),
        "checks": checks,
        "claim_scope": "automated_file_identity_manifest_metadata_rank_and_reference_routing_based_on_manually_verified_book_identity_not_pdf_text_extraction_semantic_equivalence_or_empirical_validation",
    }
...
def _normalize_artifact_token(token: str) -> str:
    ...
    if token.startswith("/mnt/data/"):
        token = token[len("/mnt/data/"):]
    return Path(token).name


def artifact_reference_audit(root: Path, bundle: dict) -> dict:
    spec = bundle.get("artifact_reference_registry", {})
    files = spec.get("semantic_files", [])
    allowed_classes = set(spec.get("allowed_nonactive_classes", []))
    registry = {
        str(item.get("artifact")): item
        for item in spec.get("nonactive_artifacts", [])
        if item.get("artifact")
    }
    active = {entry["path"] for entry in bundle.get("manifest", {}).get("active_files", [])}
    forbidden = set(spec.get("forbidden_legacy_artifacts", []))
    pattern = re.compile(
        r"(?<![\w.-])([A-Za-z0-9_./\\*?-]+\.(?:csv|json|md|py|xlsx|pdf|h5|html|png|jpg|jpeg|txt))(?![\w.-])",
        re.IGNORECASE,
    )
    refs = []
    unclassified = []
    forbidden_hits = []
    bad_registry_classes = []
    missing_files = []
    for item in registry.values():
        if item.get("classification") not in allowed_classes:
            bad_registry_classes.append(item)
    for filename in files:
        ...
        for match in pattern.finditer(text):
            raw = match.group(1)
            artifact = _normalize_artifact_token(raw)
            line = text.count("\n", 0, match.start()) + 1
            if artifact in active:
                classification = "active_manifest"
            elif artifact in registry:
                classification = registry[artifact].get("classification")
            else:
                classification = "unclassified"
                unclassified.append({"file": filename, "line": line, "artifact": artifact, "raw": raw})
            if artifact in forbidden:
                forbidden_hits.append({"file": filename, "line": line, "artifact": artifact})
            refs.append({"file": filename, "line": line, "artifact": artifact, "classification": classification})
    status = "PASS" if not (unclassified or forbidden_hits or bad_registry_classes or missing_files) else "FAIL"
    counts: dict[str, int] = {}
    for ref in refs:
    ...
    return {
        "status": status,
        "semantic_files_checked": len(files) - len(missing_files),
        "references_found": len(refs),
        "classification_counts": counts,
        "unclassified_references": unclassified,
        "forbidden_legacy_hits": forbidden_hits,
        "invalid_registry_classes": bad_registry_classes,
        "missing_semantic_files": missing_files,
        "policy": "Every file-like reference in protected semantic files must be active in manifest or explicitly classified nonactive.",
    }
...
def canonical_invariants_audit(root: Path, bundle: dict) -> dict:
    ...
    companion_name = str(bundle.get("companion_configuration", {}).get("expected_filename", ""))
    external_companion_anchor_ids = [
        str(a.get("anchor_id")) for a in anchors
        if companion_name and str(a.get("file", "")) == companion_name
    ]
    results = []
    missing_files = set()
    for anchor in anchors:
    ...
    return {
        "status": "PASS" if not missing and not external_companion_anchor_ids else "FAIL",
        ...
        "external_companion_anchor_ids_forbidden": external_companion_anchor_ids,
        "category_counts": category_counts,
        "results": results,
        "claim_scope": "canonical_corpus_semantic_presence_and_protection; external_customgpt_instruction_field_excluded_by_design",
    }
...
def audit_package(root: Path, bundle: dict) -> dict:
    ...
    grounding = module_grounding_audit(root, bundle)
    findings.append({"check": "canonical_module_grounding", "status": grounding["status"], "detail": grounding})
    counterfind = retrieval_counterfind_audit(root, bundle)
    findings.append({"check": "retrieval_counterfind_architecture", "status": counterfind["status"], "detail": counterfind})
    synthesis_guidance = synthesis_guidance_audit(root, bundle)
    findings.append({"check": "answer_synthesis_guidance", "status": synthesis_guidance["status"], "detail": synthesis_guidance})
    correspondence = instruction_correspondence_audit(root, bundle)
    findings.append({"check": "instruction_correspondence", "status": correspondence["status"], "detail": correspondence})
    ...
    book_reference = public_book_reference_audit(root, bundle)
    findings.append({"check": "public_book_reference", "status": book_reference["status"], "detail": book_reference})
    artifact_references = artifact_reference_audit(root, bundle)
    findings.append({"check": "artifact_references", "status": artifact_references["status"], "detail": artifact_references})
    research_scenarios = research_scenario_contract_audit(root, bundle)
    findings.append({"check": "research_scenario_enforcement", "status": research_scenarios["status"], "detail": research_scenarios})
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(description=f"TSM compact operations runner {RUNNER_VERSION}")
    ...
    module_grounding_cmd = sub.add_parser("module-grounding-audit")
    retrieval_counterfind_cmd = sub.add_parser("retrieval-counterfind-audit")
    synthesis_guidance_cmd = sub.add_parser("synthesis-guidance-audit")
    instruction_correspondence_cmd = sub.add_parser("instruction-correspondence-audit")
    ...
    book_reference_cmd = sub.add_parser("book-reference-audit")
    book_reference_cmd.add_argument("--out", default=None)
    artifact_reference_cmd = sub.add_parser("artifact-reference-audit")
    artifact_reference_cmd.add_argument("--out", default=None)
    research_scenario_cmd = sub.add_parser("research-scenario-evaluate")
    research_scenario_cmd.add_argument("--input", required=True)
    research_scenario_cmd.add_argument("--output", required=True)
    ai_order_cmd.add_argument("--out", default=None)
    module_grounding_cmd.add_argument("--out", default=None)
    retrieval_counterfind_cmd.add_argument("--out", default=None)
    synthesis_guidance_cmd.add_argument("--out", default=None)
    instruction_correspondence_cmd.add_argument("--out", default=None)
    ...
    if args.cmd == "validate":
        ...
        result["package_validation_status"] = "PASS"
        if args.companion:
            companion_result = companion_configuration_audit(root, bundle, args.companion)
            result["companion_audit"] = companion_result
            result["full_gpt_configuration_status"] = "PASS" if companion_result["status"] == "PASS" else "FAIL"
            ...
            result["full_gpt_configuration_status"] = "NOT_CHECKED_EXTERNAL_COMPANION"
    ...
    if args.cmd == "ai-execution-order-audit":
        ...
        emit(result, args.out); return 0 if result["status"] != "FAIL" else 2
    if args.cmd == "module-grounding-audit":
        result = module_grounding_audit(root, bundle)
        emit(result, args.out); return 0 if result["status"] != "FAIL" else 2
    if args.cmd == "retrieval-counterfind-audit":
        result = retrieval_counterfind_audit(root, bundle)
        emit(result, args.out); return 0 if result["status"] != "FAIL" else 2
    if args.cmd == "synthesis-guidance-audit":
        result = synthesis_guidance_audit(root, bundle)
        emit(result, args.out); return 0 if result["status"] != "FAIL" else 2
    if args.cmd == "instruction-correspondence-audit":
        result = instruction_correspondence_audit(root, bundle)
        emit(result, args.out); return 0 if result["status"] != "FAIL" else 2
    ...
    if args.cmd == "text-integrity-audit":
        ...
        emit(result, args.out); return 0 if result["status"] != "FAIL" else 2
    if args.cmd == "book-reference-audit":
        result = public_book_reference_audit(root, bundle)
        emit(result, args.out); return 0 if result["status"] != "FAIL" else 2
    if args.cmd == "artifact-reference-audit":
        result = artifact_reference_audit(root, bundle)
        emit(result, args.out); return 0 if result["status"] != "FAIL" else 2
    if args.cmd == "research-scenario-evaluate":
        rows = research_scenario_rows(
            load_records(root / args.input), profile, read_rows(root / profile["paths"]["core_data"])
        )
        write_records(root / args.output, rows)
        result = _simple_command_result(rows, "research_scenario_status")
        result["classification_allowed"] = False
        result["calculation_status"] = "DEMO_ONLY"
        emit(result); return 0 if not any(k.startswith("BLOCKED_INPUT_CONTRACT") for k in result["status_counts"]) else 2
