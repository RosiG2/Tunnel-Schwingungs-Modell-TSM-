#!/usr/bin/env python3
"""TSM compact operations runner v8.0.
...
def validate_operations(bundle: dict) -> tuple[dict, dict]:
    ...
    counterfind_policy = require(ai_system, "retrieval_counterfind_policy")
    if counterfind_policy.get("required_for_material_tsm_synthesis") is not True:
        raise ValueError("Material TSM syntheses require an independent corpus-wide counterfind pass.")
    if counterfind_policy.get("independent_from_primary_module_graph") is not True:
        raise ValueError("The counterfind pass must leave the primary module graph.")
    if counterfind_policy.get("alternative_semantic_axis_required") is not True:
        raise ValueError("The counterfind pass requires an alternative semantic search axis.")
    if counterfind_policy.get("exhaustive_chunk_retrieval_guarantee_claim_forbidden") is not True:
        raise ValueError("The corpus may not claim a technical exhaustive-chunk retrieval guarantee.")
    if counterfind_policy.get("additional_pass_only_on_new_material_signal") is not True:
        raise ValueError("Additional counterfind passes require a new material signal.")

    synthesis_policy = require(ai_system, "answer_synthesis_policy")
    if synthesis_policy.get("synthesis_primacy") is not True:
        raise ValueError("Open material TSM questions require synthesis primacy.")
    if synthesis_policy.get("provisional_relational_synthesis_before_counterfind") is not True:
        raise ValueError("A provisional relational synthesis must precede the counterfind pass.")
...
def ai_execution_order_audit(root: Path, bundle: dict) -> dict:
    ...
    try:
        ...
        key_markers = [
            ...
            "Eine materielle TSM-Antwort soll die TSM-Perspektive zuerst wirklich arbeiten lassen",
        ...
        expected_workflow = [
            ...
            "identify_epistemic_goal_retrieve_actual_modules_and_form_provisional_relational_synthesis",
            "perform_independent_corpuswide_counterfind_pass_for_material_synthesis",
            "integrate_material_counterfinds_and_deepen_only_material_paths",
            "check_subject_focus_and_relational_process_value",
            "determine_internal_claim_and_source_status",
            "condense_checked_synthesis_and_offer_material_connection_preview_when_substantive",
        ]
        ...
        expected_statuses = [
            "BASIS_WIEDERGABE", "BASIS_SYNTHESE", "BUCHDARSTELLUNG_ODER_BUCHSYNTHESE",
            "ABGELEITETE_TSM_DEUTUNG", "OPERATIVE_ANWENDUNG", "EXTERNE_FACHBEHAUPTUNG",
        ]
        ...
        checks["claim_status_markdown_present"] = all(marker in key_text for marker in [
            "BASIS-Wiedergabe", "BASIS-Synthese", "Buchdarstellung", "Abgeleitete TSM-Deutung",
        ...
        required_true = {
            ...
            "claim_status_is_internal_by_default",
            "visible_claim_labels_only_when_material",
            "normal_tsm_answer_is_coherent_unit",
            "book_perspective_variation_need_not_be_announced_unless_material",
            "legacy_source_labeling_not_required_in_normal_dialogue",
            "exploratory_e1_e2_modeling_allowed_with_model_set_proxies",
            "fail_closed_limits_claim_not_exploration",
            "independent_counterfind_pass_required_for_material_synthesis",
            "counterfind_must_leave_primary_module_graph",
            "counterfind_alternative_semantic_axis_required",
            "counterfind_exhaustive_retrieval_claim_forbidden",
            "counterfind_resultless_repetition_forbidden",
            "synthesis_primacy_for_open_material_questions",
            "provisional_relational_synthesis_precedes_counterfind",
            "counterfind_improves_synthesis_not_default_visible_outline",
            "counterfind_visible_only_when_material_to_result",
            "subject_focus_preserved_unless_secondary_path_has_greater_material_value",
            "relational_process_value_preferred_over_module_or_term_listing_when_supported",
            "material_connection_preview_optional",
            "connection_preview_must_add_new_distinction_or_next_step",
            "research_connection_preview_prefers_observable_comparison_countertest_or_falsifier",
            "routine_generic_connection_preview_forbidden",
        }
        ...
        checks["claim_label_economy_present"] = (
            "primär eine interne Wahrheits-, Herkunfts- und Geltungsordnung" in key_text
            and "Geltungsstatus intern immer bestimmen, sichtbar nur bei materieller Bedeutung" in key_text
        )
        ...
        checks["book_perspective_internal_status_present"] = (
            "Buchdarstellung oder Buchsynthese" in key_text
            and "engere oder breitere Buchperspektive" in key_text
        )
        checks["legacy_source_labeling_bounded"] = (
            "keine Pflicht" in key_text
            and "normale dialogische TSM-Antworten" in key_text
        )
        checks["exploratory_research_policy_present"] = (
            policies.get("exploratory_e1_e2_modeling_allowed_with_model_set_proxies") is True
            and policies.get("fail_closed_limits_claim_not_exploration") is True
        )
        counterfind = system.get("retrieval_counterfind_policy", {})
        checks["counterfind_policy_bound_into_execution_order"] = (
            counterfind.get("required_for_material_tsm_synthesis") is True
            and counterfind.get("independent_from_primary_module_graph") is True
            and counterfind.get("alternative_semantic_axis_required") is True
            and "Korpusweite Gegenfundprüfung" in key_text
        )
        synthesis_policy = system.get("answer_synthesis_policy", {})
        checks["synthesis_primacy_bound_into_execution_order"] = (
            synthesis_policy.get("synthesis_primacy") is True
            and synthesis_policy.get("provisional_relational_synthesis_before_counterfind") is True
            and "Syntheseprimat" in key_text
        )
        checks["process_value_and_optional_connection_preview_present"] = (
            synthesis_policy.get("subject_primacy") is True
            and synthesis_policy.get("connection_preview", {}).get("mandatory") is False
            and "Gegenstands- und Prozessprimat" in key_text
            and "Anschlussvorschau" in key_text
        )
...
def retrieval_counterfind_audit(root: Path, bundle: dict) -> dict:
    """Audit two-pass semantic retrieval rules without claiming visibility into platform retrieval internals."""
    checks: dict[str, bool] = {}
    details: dict[str, object] = {}
    try:
        system = bundle.get("ai_synthesis_system", {})
        ...
        key_text = key_path.read_text(encoding="utf-8") if key_path.is_file() else ""
        workflow = system.get("workflow", [])
        authority_files = bundle.get("semantic_authority", {}).get("files", [])
        active_files = {str(x.get("path", "")) for x in bundle.get("manifest", {}).get("active_files", [])}

        checks["policy_registry_present"] = bool(policy)
        checks["policy_schema_v1"] = policy.get("schema_id") == "tsm.retrieval-counterfind-policy.v1"
        checks["required_for_material_synthesis"] = policy.get("required_for_material_tsm_synthesis") is True
        checks["simple_definition_exception_present"] = policy.get("not_required_for_simple_wording_locator_or_definition_question") is True
        checks["minimum_independent_passes_one"] = int(policy.get("minimum_independent_counterfind_passes", 0)) == 1
        checks["independent_from_primary_graph"] = policy.get("independent_from_primary_module_graph") is True
        checks["alternative_axis_required"] = policy.get("alternative_semantic_axis_required") is True
        expected_axes = {"counterterm", "scale", "process_function", "time_direction", "domain_connection", "effect_level"}
        checks["alternative_axes_catalogue_sufficient"] = expected_axes.issubset(set(policy.get("alternative_semantic_axes", [])))
        expected_targets = {"contradicting", "limiting", "materially_complementary", "differently_scaled", "differently_termed"}
        checks["counterfind_targets_complete"] = expected_targets.issubset(set(policy.get("counterfind_targets", [])))
        checks["additional_pass_only_on_new_material_signal"] = policy.get("additional_pass_only_on_new_material_signal") is True
        checks["resultless_repetition_forbidden"] = policy.get("resultless_repetition_opens_no_new_loop") is True
        checks["exhaustive_retrieval_claim_forbidden"] = policy.get("exhaustive_chunk_retrieval_guarantee_claim_forbidden") is True
        checks["visible_search_trace_not_required"] = policy.get("visible_search_trace_required") is False
        checks["semantic_authority_scope_registered"] = policy.get("authority_scope") == "semantic_authority.files"
        checks["semantic_authority_files_are_active"] = bool(authority_files) and all(str(x) in active_files for x in authority_files)

        step = "perform_independent_corpuswide_counterfind_pass_for_material_synthesis"
        provisional = "identify_epistemic_goal_retrieve_actual_modules_and_form_provisional_relational_synthesis"
        integrate = "integrate_material_counterfinds_and_deepen_only_material_paths"
        checks["workflow_contains_counterfind_step"] = step in workflow
        checks["workflow_contains_provisional_synthesis_step"] = provisional in workflow
        checks["workflow_contains_material_integration_step"] = integrate in workflow
        if step in workflow and provisional in workflow and integrate in workflow:
            checks["workflow_counterfind_position_correct"] = (
                workflow.index(provisional) < workflow.index(step) < workflow.index(integrate)
            )
            checks["provisional_synthesis_precedes_counterfind"] = workflow.index(provisional) < workflow.index(step)
        else:
            checks["workflow_counterfind_position_correct"] = False
            checks["provisional_synthesis_precedes_counterfind"] = False

        required_markers = [
            "**Korpusweite Gegenfundprüfung:** Bei einer materiellen TSM-Synthese endet die Suche nicht mit dem zuerst schlüssigen Modulpfad.",
            "Der Gegenfundlauf beginnt erneut beim Gegenstand und darf nicht lediglich dieselben Suchbegriffe, Modulnummern oder bereits verfolgten Verbindungen wiederholen.",
            "Er muss mindestens einen alternativen semantischen Zugang verwenden",
            "Nach mindestens einem unabhängigen Gegenfundlauf werden weitere Suchpfade nur verfolgt, wenn ein neuer materieller Hinweis entstanden ist.",
            "keine serielle Volltextabarbeitung jedes Moduls",
        ]
        for idx, text in enumerate(required_markers, 1):
            checks[f"reading_key_marker_{idx:02d}"] = text in key_text
        checks["integrity_check_includes_counterfind"] = "mindestens ein unabhängiger korpusweiter Gegenfundlauf aus einer alternativen semantischen Richtung" in key_text

        details.update({
            "authority_files": authority_files,
            "alternative_semantic_axes": policy.get("alternative_semantic_axes", []),
            "counterfind_targets": policy.get("counterfind_targets", []),
            "workflow": workflow,
            "note": "This audit verifies the mandated two-pass retrieval architecture and stop rules. It cannot prove which chunks the platform retrieval layer actually fetched in a live prompt.",
        })
    except Exception as exc:
        return {"status": "FAIL", "checks": checks, "details": details, "error": str(exc)}
    status = "PASS" if checks and all(checks.values()) else "FAIL"
    return {
        ...
        "checks": checks,
        "details": details,
        "claim_scope": "retrieval_architecture_and_counterfind_policy_presence_not_live_platform_retrieval_proof",
    }


def synthesis_guidance_audit(root: Path, bundle: dict) -> dict:
    """Audit synthesis primacy, subject/process value and optional material connection-preview governance."""
    checks: dict[str, bool] = {}
    details: dict[str, object] = {}
    try:
        system = bundle.get("ai_synthesis_system", {})
        ...
        checks["policy_registry_present"] = bool(policy)
        checks["policy_schema_v1"] = policy.get("schema_id") == "tsm.answer-synthesis-guidance-policy.v1"
        checks["synthesis_primacy"] = policy.get("synthesis_primacy") is True
        checks["provisional_synthesis_before_counterfind"] = policy.get("provisional_relational_synthesis_before_counterfind") is True
        ...
        checks["research_connection_preference_testable"] = connection.get("research_preference") == "what_can_next_be_observed_compared_or_falsified"
        checks["generic_closing_forbidden"] = connection.get("generic_more_research_needed_closing_forbidden") is True

        provisional = "identify_epistemic_goal_retrieve_actual_modules_and_form_provisional_relational_synthesis"
        counterfind = "perform_independent_corpuswide_counterfind_pass_for_material_synthesis"
        integrate = "integrate_material_counterfinds_and_deepen_only_material_paths"
        process_step = "check_subject_focus_and_relational_process_value"
        final_step = "condense_checked_synthesis_and_offer_material_connection_preview_when_substantive"
        required_steps = [provisional, counterfind, integrate, process_step, final_step]
        checks["workflow_steps_present"] = all(x in workflow for x in required_steps)
        if checks["workflow_steps_present"]:
            checks["workflow_order_synthesis_counterfind_integration_process_output"] = (
                workflow.index(provisional) < workflow.index(counterfind) < workflow.index(integrate)
                < workflow.index(process_step) < workflow.index(final_step)
            )
        else:
            checks["workflow_order_synthesis_counterfind_integration_process_output"] = False

        required_markers = [
            "**Syntheseprimat:** Bei offenen Fragen wie „Was sagt das TSM dazu?“ wird nach Klärung des Gegenstands zunächst die stärkste tragfähige relationale TSM-Synthese entwickelt.",
            "**Ein Gegenfund ist kein Ausgabeziel:** Er wird sichtbar, wenn er die TSM-Synthese materiell verändert, begrenzt, korrigiert oder eine relevante Alternative eröffnet.",
            "**Gegenstands- und Prozessprimat:** Eine TSM-Synthese soll den Gegenstand nicht zugunsten methodischer, symbolischer oder erkenntnistheoretischer Nebenpfade verlassen",
            "**Anschlussvorschau:** Ergibt sich aus einer materiellen TSM-Synthese ein eigenständiger nächster Erkenntnis-, Prüf- oder Handlungsschritt, kann die Antwort ihn knapp sichtbar machen.",
            "Die Anschlussvorschau ist kein Pflichtabschluss und darf das bereits Gesagte nicht bloß umformulieren.",
        ]
        for idx, text in enumerate(required_markers, 1):
            checks[f"reading_key_marker_{idx:02d}"] = text in key_text
        ...
        details.update({
            "workflow": workflow,
            "visible_answer_order": policy.get("visible_answer_order", []),
            "candidate_connection_forms": connection.get("candidate_forms", []),
            "note": "This audit verifies synthesis/output-guidance architecture. It does not prove live model response quality; Preview regressions remain required.",
        })
    except Exception as exc:
        return {"status": "FAIL", "checks": checks, "details": details, "error": str(exc)}
    status = "PASS" if checks and all(checks.values()) else "FAIL"
    return {
        ...
        "checks": checks,
        "details": details,
        "claim_scope": "answer_synthesis_guidance_architecture_not_live_model_behavior_proof",
    }
...
def dialog_routing_audit(root: Path, bundle: dict) -> dict:
    ...
    try:
        ...
        checks["workflow_ends_with_compact_integrity_and_condensation"] = workflow[-1:] == ["condense_checked_synthesis_and_offer_material_connection_preview_when_substantive"]
